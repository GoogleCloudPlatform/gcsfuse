...
The trace explorer provides a view of the underlying spans generated by GCSFuse, allowing you to filter spans and traces for the current GCSFuse instance. Filtering options include the instance ID (as previously mentioned) and other Google Cloud attributes such as the host type (machine type). These attributes can be used to filter and group traces, providing more insight into performance. For example, you can compare performance across different machine types.
