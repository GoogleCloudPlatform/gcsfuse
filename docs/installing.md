...
Download and install the latest release package:
    ...
    curl -L -O https://github.com/GoogleCloudPlatform/gcsfuse/releases/download/v0.42.2/gcsfuse-0.42.2-1.x86_64.rpm
    sudo rpm --install -p gcsfuse-0.42.2-1.x86_64.rpm
...
If you are on a distribution that uses `.rpm` files for package management:
    ...
    curl -L -O https://github.com/GoogleCloudPlatform/gcsfuse/releases#:~:text=gcsfuse%2D0.42.2%2D1.x86_64.rpm
    sudo rpm --install -p gcsfuse-0.42.2-1.x86_64.rpm

Or one that uses `.deb` files:

    curl -L -O https://github.com/GoogleCloudPlatform/gcsfuse/releases/download/v0.42.2/gcsfuse_0.42.2_amd64.deb
    sudo dpkg --install gcsfuse_0.42.2_amd64.deb
