...
1. **Type-cache capacity**: This is configurable at per-directory level by setting `metadata-cache: type-cache-max-size-mb` in config-file. This is the maximum size of type-cache per-directory in MiBs. By default, this is set at 80, which roughly equates to about a million entries (based on 80 bytes per [type-cache entry](https://github.com/GoogleCloudPlatform/gcsfuse/blob/acdb559a7b18e6c863db35b54c463fedd3fd1032/internal/cache/metadata/type_cache.go#L86)).
