...
func BuildFlagSet(flagSet *pflag.FlagSet) error {
  ...
  flagSet.IntP("file-cache-sparse-file-chunk-size-mb", "", 1, "Size of chunks in MiB for sparse file downloads. Only effective when enable-sparse-file is true.")
  
  
  if err := flagSet.MarkHidden("file-cache-sparse-file-chunk-size-mb"); err != nil {
    return err
  }
...
func BindFlags(v *viper.Viper, flagSet *pflag.FlagSet) error {
  ...
  if err := v.BindPFlag("file-cache.sparse-file-chunk-size-mb", flagSet.Lookup("file-cache-sparse-file-chunk-size-mb")); err != nil {
    return err
  }
