...
func BuildFlagSet(flagSet *pflag.FlagSet) error {
  ...
  flagSet.StringP("metrics-project-id", "", "", "Specify the GCP project id to which metrics will be exported. When unset, a project id will be inferred as per the default credential detection process")
  
  
  if err := flagSet.MarkHidden("metrics-project-id"); err != nil {
    return err
  }
...
func BindFlags(v *viper.Viper, flagSet *pflag.FlagSet) error {
  ...
  if err := v.BindPFlag("metrics.project-id", flagSet.Lookup("metrics-project-id")); err != nil {
    return err
  }
