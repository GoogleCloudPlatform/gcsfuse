	...
	BucketTypeOptimization: []shared.BucketTypeOptimization{
		{
			...
			Value: int64(BucketTypeZonal.DefaultCongestionThreshold()),
		},
		{
			BucketTypes: shared.BucketTypeList{
				"flat",
			...
			Value: int64(BucketTypeHierarchical.DefaultCongestionThreshold()),
		},
	},
	...
	BucketTypeOptimization: []shared.BucketTypeOptimization{
		{
			...
			Value: int64(BucketTypeHierarchical.DefaultFuseMaxRequestSizeKb()),
		},
	},
}, "implicit-dirs": {
	...
	BucketTypeOptimization: []shared.BucketTypeOptimization{
		{
			...
			Value: int64(BucketTypeZonal.DefaultMaxBackground()),
		},
		{
			BucketTypes: shared.BucketTypeList{
				"flat",
			...
			Value: int64(BucketTypeHierarchical.DefaultMaxBackground()),
		},
	},
	...
	BucketTypeOptimization: []shared.BucketTypeOptimization{
		{
			...
			Value: int64(BucketTypeZonal.DefaultMaxReadAheadKb()),
		},
		{
			BucketTypes: shared.BucketTypeList{
				"flat",
			...
			Value: int64(BucketTypeHierarchical.DefaultMaxReadAheadKb()),
		},
	},
...
func BuildFlagSet(flagSet *pflag.FlagSet) error {
	...
	flagSet.IntP("fuse-max-request-size-kb", "", BucketTypeZonal.DefaultFuseMaxRequestSizeKb(), "Sets the target maximum request size in KiB that FUSE can process in a single request (currently used to control read requests only). This is translated to the kernel max_pages limit based on host page size. As max_pages_limit is a global, machine-level configuration across all mounts, the host's limit is only updated if the calculated pages value is greater than the current system limit. Note that the FUSE kernel max_pages limit can be set to at most 65535 (fuse_max_max_pages), so values exceeding 65535 pages are capped at 65535.  Additionally, on GKE, the system-wide setting is capped to 16 MiB (16384 KiB) by default by the CSI driver. If needed to be set beyond that on GKE, the user has to manually increase the value on the node before GCSFuse mounting begins.")
