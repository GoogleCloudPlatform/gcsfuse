...
type FileSystemConfig struct {
  ...
  ExperimentalEnablePassthrough bool `yaml:"experimental-enable-passthrough"`
...
func BuildFlagSet(flagSet *pflag.FlagSet) error {
  ...
  flagSet.BoolP("experimental-enable-passthrough", "", false, "Enables FUSE passthrough for read/write operations (experimental, Linux kernel >= 6.9).")
  
  
  if err := flagSet.MarkHidden("experimental-enable-passthrough"); err != nil {
    return err
  }
...
func BindFlags(v *viper.Viper, flagSet *pflag.FlagSet) error {
  ...
  if err := v.BindPFlag("file-system.experimental-enable-passthrough", flagSet.Lookup("experimental-enable-passthrough")); err != nil {
    return err
  }
