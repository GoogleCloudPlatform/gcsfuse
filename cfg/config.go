...
func (c *Config) ApplyOptimizations(isSet IsValueSet, input ...OptimizationInput) map[string]OptimizationResult {
	...
	if !isSet.IsSet("enable-kernel-reader") {
		...
		result := getOptimizedValue(&rules, c.FileSystem.EnableKernelReader, profileName, machineType, optimizationInput, machineTypeToGroupMap)
		if result.Optimized {
	...
	if !isSet.IsSet("file-cache-cache-file-for-range-read") {
		...
		result := getOptimizedValue(&rules, c.FileCache.CacheFileForRangeRead, profileName, machineType, optimizationInput, machineTypeToGroupMap)
		if result.Optimized {
			if val, ok := result.FinalValue.(bool); ok {
	...
	if !isSet.IsSet("implicit-dirs") {
		...
		result := getOptimizedValue(&rules, c.ImplicitDirs, profileName, machineType, optimizationInput, machineTypeToGroupMap)
		if result.Optimized {
			if val, ok := result.FinalValue.(bool); ok {
	...
	if !isSet.IsSet("kernel-list-cache-ttl-secs") {
		...
		result := getOptimizedValue(&rules, c.FileSystem.KernelListCacheTtlSecs, profileName, machineType, optimizationInput, machineTypeToGroupMap)
		if result.Optimized {
	...
	if !isSet.IsSet("metadata-cache-negative-ttl-secs") {
		...
		result := getOptimizedValue(&rules, c.MetadataCache.NegativeTtlSecs, profileName, machineType, optimizationInput, machineTypeToGroupMap)
		if result.Optimized {
			if val, ok := result.FinalValue.(int64); ok {
	...
	if !isSet.IsSet("metadata-cache-ttl-secs") {
		...
		result := getOptimizedValue(&rules, c.MetadataCache.TtlSecs, profileName, machineType, optimizationInput, machineTypeToGroupMap)
		if result.Optimized {
			if val, ok := result.FinalValue.(int64); ok {
	...
	if !isSet.IsSet("rename-dir-limit") {
		...
		result := getOptimizedValue(&rules, c.FileSystem.RenameDirLimit, profileName, machineType, optimizationInput, machineTypeToGroupMap)
		if result.Optimized {
			if val, ok := result.FinalValue.(int64); ok {
	...
	if !isSet.IsSet("stat-cache-max-size-mb") {
		...
		result := getOptimizedValue(&rules, c.MetadataCache.StatCacheMaxSizeMb, profileName, machineType, optimizationInput, machineTypeToGroupMap)
		if result.Optimized {
			if val, ok := result.FinalValue.(int64); ok {
	...
	if !isSet.IsSet("type-cache-max-size-mb") {
		...
		result := getOptimizedValue(&rules, c.MetadataCache.TypeCacheMaxSizeMb, profileName, machineType, optimizationInput, machineTypeToGroupMap)
		if result.Optimized {
			if val, ok := result.FinalValue.(int64); ok {
	...
	if !isSet.IsSet("write-global-max-blocks") {
		...
		result := getOptimizedValue(&rules, c.Write.GlobalMaxBlocks, profileName, machineType, optimizationInput, machineTypeToGroupMap)
		if result.Optimized {
			if val, ok := result.FinalValue.(int64); ok {
