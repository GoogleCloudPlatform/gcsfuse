...
type GcsConnectionConfig struct {
  ...
  ExperimentalEnableGrpcBufferPool bool `yaml:"experimental-enable-grpc-buffer-pool"`
...
func BuildFlagSet(flagSet *pflag.FlagSet) error {
  ...
  flagSet.BoolP("experimental-enable-grpc-buffer-pool", "", false, "Enable the experimental gRPC shared buffer pool using gomem block-pool.")
  
  
  if err := flagSet.MarkHidden("experimental-enable-grpc-buffer-pool"); err != nil {
    return err
  }
...
func BindFlags(v *viper.Viper, flagSet *pflag.FlagSet) error {
  ...
  if err := v.BindPFlag("gcs-connection.experimental-enable-grpc-buffer-pool", flagSet.Lookup("experimental-enable-grpc-buffer-pool")); err != nil {
    return err
  }
