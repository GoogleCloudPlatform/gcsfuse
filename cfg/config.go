...
func BindFlags(flagSet *pflag.FlagSet) (v *viper.Viper, err error) {
	...
	err = v.BindPFlag("file-cache.download-parallelism-per-file", flagSet.Lookup("download-parallelism-per-file"))
	if err != nil {
		return nil, err
	}
	...
	err = v.BindPFlag("file-cache.enable-crc-check", flagSet.Lookup("enable-crc-check"))
	if err != nil {
		return nil, err
	}
	...
	err = v.BindPFlag("file-cache.max-download-parallelism", flagSet.Lookup("max-download-parallelism"))
	if err != nil {
		return nil, err
	}
	...
	err = v.BindPFlag("file-cache.read-request-size-mb", flagSet.Lookup("read-request-size-mb"))
	if err != nil {
		return nil, err
	}
