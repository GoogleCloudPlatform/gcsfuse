...
const (
	...
	// EnsureDirectPathIfEligible falls back to HTTP/1 when DirectPath is impossible
	// (e.g., non-GCE VMs, cross-region VM/bucket), but fails when DirectPath is
	// possible but persistently broken.
	EnsureDirectPathIfEligible DirectPathStrategy = "ensure-direct-path-if-eligible"
)

func (d *DirectPathStrategy) UnmarshalText(text []byte) error {
	txtStr := string(text)
	strategy := strings.ToLower(txtStr)
	v := []string{"direct-path-only", "direct-path-with-fallback", "ensure-direct-path-if-eligible"}
