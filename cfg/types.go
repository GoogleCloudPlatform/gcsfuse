...
// Protocol is the datatype that specifies the type of connection: http1/http2/grpc/httpwithmtls.
...
const (
	...
	HTTP2        = "http2"
	GRPC         = "grpc"
	HTTPWithMtls = "httpwithmtls"
)

func (p *Protocol) UnmarshalText(text []byte) error {
	txtStr := string(text)
	protocol := strings.ToLower(txtStr)
	v := []string{"http1", "http2", "grpc", "httpwithmtls"}
