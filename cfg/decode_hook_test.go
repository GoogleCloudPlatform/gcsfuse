...
func TestParsingError(t *testing.T) {
	...
	declareFlags := func() *flag.FlagSet {
		...
		fs.String("logSeverityParam", "INFO", "")
		fs.String("protocolParam", "http1", "")
		fs.String("directPathStrategyParam", "ensure-direct-path-if-eligible", "")
		return fs
	}
	bindFlags := func(fs *flag.FlagSet) *viper.Viper {
		t.Helper()
	...
	tests := []struct {
		...
		},
		{
			...
			errMsg: "invalid direct-path strategy value: invalid-strategy. It can only accept values in the list: [direct-path-only direct-path-with-fallback ensure-direct-path-if-eligible]",
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
