...
func (c *grpcStorageClient) getBandwidthLimiter() *rate.Limiter {
		...
		if os.Getenv("GCS_DUMMY_MRD") == "true" || os.Getenv("GCS_DUMMY_MRD") == "1" {
			...
			if val := os.Getenv("GCS_DUMMY_MRD_MB_PER_SEC"); val != "" {
				if parsed, err := strconv.Atoi(val); err == nil {
					if parsed <= 0 {
						// Return without initializing bandwidthLimiter so rate limiting is disabled.
						return
					}
					limitMB = parsed
				}
...
func (d *readResponseDecoder) writeToAndUpdateCRC(w io.Writer, readID int64, updateCRC func([]byte)) (totalWritten int64, found bool, err error) {
	if d.isDummy {
		length, ok := d.dummyLengths[readID]
		if !ok || length <= 0 {
			return 0, false, nil
		}
		written, err := io.Copy(w, io.LimitReader(zeroReader{}, length))
