...
func newBidiReadStreamSession(ctx context.Context, id int, respC chan<- mrdSessionResult, client *grpcStorageClient, settings *settings, params *newMultiRangeDownloaderParams, readSpec *storagepb.BidiReadObjectSpec) (*bidiReadStreamSession, error) {
	...
	if os.Getenv("GCS_DUMMY_MRD") == "true" || os.Getenv("GCS_DUMMY_MRD") == "1" {
		s.stream = newDummyBidiStream(s.ctx, client.getBandwidthLimiter())
	} else {
		var err error
...
func (s *bidiReadStreamSession) receiveLoop() {
	...
	for {
		...
		if err == nil {
			if ds, ok := s.stream.(*dummyBidiStream); ok {
				decoder = ds.popDecoder()
			} else {
...
// --- Dummy MRD Bidi Stream for Benchmarking ---
	...
	for {
		...
		case req := <-d.reqQueue:
			...
			if totalBytes > 0 {
				...
				if d.limiter != nil {
					bytesLeft := int(totalBytes)
					burst := d.limiter.Burst()
					for bytesLeft > 0 {
						waitBytes := bytesLeft
						if burst > 0 && waitBytes > burst {
							waitBytes = burst
						}
						if err := d.limiter.WaitN(d.ctx, waitBytes); err != nil {
							if d.debug {
...
func (d *dummyBidiStream) CloseSend() error {
	return nil
}

func (d *dummyBidiStream) Recv() (*storagepb.BidiReadObjectResponse, error) {
	return nil, errors.New("Recv() not used in custom codec mode")
}
