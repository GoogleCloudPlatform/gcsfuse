...
func (rx *ResumableUpload) uploadChunkWithRetries(ctx context.Context, chunk io.Reader, off, size int64, done bool) (resp *http.Response, err error) {
	log.Printf("GCSFuse-Debug: uploadChunkWithRetries start, off: %d, size: %d, done: %t", off, size, done)
	defer func() {
		log.Printf("GCSFuse-Debug: uploadChunkWithRetries end, off: %d, size: %d, done: %t, err: %v", off, size, done, err)
	}()
