...
func (rx *ResumableUpload) transferChunk(ctx context.Context) (*http.Response, error) {
	startTime := time.Now()
	fmt.Printf("DEBUG: Before Media read: InvocationID=%s Attempt=%d StartTime=%s", rx.invocationID, rx.attempts, startTime.Format(time.RFC3339))

	chunk, off, size, err := rx.Media.Chunk()

	startTime = time.Now()
	fmt.Printf("DEBUG: After Media read: InvocationID=%s Attempt=%d StartTime=%s", rx.invocationID, rx.attempts, startTime.Format(time.RFC3339))
	...
	startTime = time.Now()
	fmt.Printf("DEBUG: After doUploadRequest function: InvocationID=%s Attempt=%d StartTime=%s", rx.invocationID, rx.attempts, startTime.Format(time.RFC3339))
...
func (rx *ResumableUpload) Upload(ctx context.Context) (resp *http.Response, err error) {
	...
	startTime := time.Now()
	fmt.Printf("DEBUG: Starting chunk upload: InvocationID=%s Attempt=%d StartTime=%s", rx.invocationID, rx.attempts, startTime.Format(time.RFC3339))
	// Send all chunks.
	for {
		...
		startTime := time.Now()
		fmt.Printf("DEBUG: Starting new chunk upload: InvocationID=%s Attempt=%d StartTime=%s", rx.invocationID, rx.attempts, startTime.Format(time.RFC3339))

		// Retry loop for a single chunk.
		for {
			startTime := time.Now()
			fmt.Printf("DEBUG: In inner for loop: InvocationID=%s Attempt=%d StartTime=%s", rx.invocationID, rx.attempts, startTime.Format(time.RFC3339))
			...
			startTime = time.Now()
			fmt.Printf("DEBUG: After creating transfer timeout: InvocationID=%s Attempt=%d StartTime=%s", rx.invocationID, rx.attempts, startTime.Format(time.RFC3339))
			...
			startTime = time.Now()
			fmt.Printf("DEBUG: After resp body cleap up: InvocationID=%s Attempt=%d StartTime=%s", rx.invocationID, rx.attempts, startTime.Format(time.RFC3339))

			resp, err = rx.transferChunk(rCtx)

			startTime = time.Now()
			fmt.Printf("DEBUG: After transferChunk function: InvocationID=%s Attempt=%d StartTime=%s", rx.invocationID, rx.attempts, startTime.Format(time.RFC3339))
