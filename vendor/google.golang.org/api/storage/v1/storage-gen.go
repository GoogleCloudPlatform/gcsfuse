...
func (c *ObjectsInsertCall) Do(opts ...googleapi.CallOption) (*Object, error) {
	gensupport.SetOptions(c.urlParams_, opts...)
	log.Printf("GCSFuse-Debug: ObjectsInsertCall.doRequest start for bucket: %s, name: %s", c.bucket, c.object.Name)
	res, err := c.doRequest("json")
	log.Printf("GCSFuse-Debug: ObjectsInsertCall.doRequest end for bucket: %s, name: %s, err: %v", c.bucket, c.object.Name, err)
	if res != nil && res.StatusCode == http.StatusNotModified {
		if res.Body != nil {
			res.Body.Close()
		}
