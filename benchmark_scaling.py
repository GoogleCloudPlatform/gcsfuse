...
def safe_unmount():
    try:
        run_cmd(f"fusermount -uz {MOUNT_POINT}", check=False)
    except Exception:
        pass
    try:
...
def main():
    ...
    for num_bytes, label in TARGET_BYTES_LIST:
        ...
        for iter_num in range(1, ITERATIONS + 1):
            print("\n" + "="*70)
            print(f"Starting test for: --mrd-target-pending-bytes={label} | Iteration {iter_num}/{ITERATIONS}")
            print("="*70)
            ...
            # Setup environment variables explicitly for Dummy MRD (only latency simulation active)
            env = os.environ.copy()
            env["GCS_DUMMY_MRD"] = "true"
            env["GCS_DUMMY_MRD_MB_PER_SEC"] = "-1"    # <= 0 disables token bucket rate limiting completely
            env["GCS_DUMMY_MRD_LATENCY_MS"] = "5.0"   # Simulates latency per MB (ms/MB)
            ...
            run_cmd("sudo sh -c 'echo 3 > /proc/sys/vm/drop_caches'", check=False)
            ...
            all_runs_data[label][f"iter_{iter_num}"] = {
                "stats": run_data,
                "bandwidth": fio_bw
            }
            print(f"Captured {len(run_data)} seconds of stats for Iteration {iter_num}.")
    ...
    print("\n" + "="*85)
    print("           SCALING & THROUGHPUT BENCHMARK SUMMARY TABLE (2 ITERATIONS)")
    print("="*85)
    print(f"{'Target Bytes':<12} | {'Iter':<6} | {'FIO Bandwidth':<15} | {'Max Combined Streams':<22} | {'Max Combined Pending':<22}")
    print("-"*85)
    for num_bytes, label in TARGET_BYTES_LIST:
        for iter_num in range(1, ITERATIONS + 1):
            run_info = all_runs_data.get(label, {}).get(f"iter_{iter_num}", {})
            ...
            if data:
                ...
                print(f"{label:<12} | {iter_num:<6} | {bw:<15} | {max_streams:<22} | {max_pending:<22}")
            else:
                print(f"{label:<12} | {iter_num:<6} | {bw:<15} | {'No Data':<22} | {'No Data':<22}")
    print("="*85)
    ...
    for (num_bytes, label), color in zip(TARGET_BYTES_LIST, colors):
        run_info = all_runs_data.get(label, {}).get("iter_1", {})
        data = run_info.get("stats", [])
        if not data:
            continue
