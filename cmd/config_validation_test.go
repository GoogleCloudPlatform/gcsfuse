...
func TestValidateExportIntervalSecs(t *testing.T) {
	testCases := []struct {
		name    string
		args    []string
		wantErr bool
		...
		},
		{
			...
			args:    []string{"--metrics-export-interval-secs=30"},
			wantErr: false,
		},
		{
			name:    "neg_stackdriver_export_interval",
			args:    []string{"--stackdriver-export-interval=-1h"},
			wantErr: true,
		},
		{
			name:    "neg_metrics_export_interval",
			args:    []string{"--metrics-export-interval=-50"},
			wantErr: true,
		},
		{
			name:    "both_set",
			args:    []string{"--stackdriver-export-interval=1h", "--metrics-export-interval=20"},
			wantErr: true,
		},
	}
	for _, tc := range testCases {
...
func TestValidateConfigFile_MetricsConfigInvalid(t *testing.T) {
	testCases := []struct {
		...
		{
			name:       "neg_export_interval",
			configFile: "testdata/invalid_metrics_config_neg.yaml",
		},
	}
