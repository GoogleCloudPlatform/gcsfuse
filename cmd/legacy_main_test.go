...
func (t *MainTest) TestGetUserAgentWithConfig_SanitizationAndSerialization() {
	...
	expectedConfigProto, err := mountConfig.SerializeConfigToProtoBase64()
	require.NoError(t.T(), err)
	expectedUserAgent := strings.TrimSpace(fmt.Sprintf("gcsfuse/%s AppName (GPN:gcsfuse-DLVM) (GCSFuseConfig:%s) (mount-id:testFS-123)", common.GetVersion(), expectedConfigProto))
	assert.Equal(t.T(), expectedUserAgent, userAgent)
}
...
func (t *MainTest) TestGetUserAgentWithConfig_WithoutEnvVar() {
	...
	expectedConfigProto, err := mountConfig.SerializeConfigToProtoBase64()
	require.NoError(t.T(), err)
	expectedUserAgent := strings.TrimSpace(fmt.Sprintf("gcsfuse/%s (GPN:gcsfuse-AppName) (GCSFuseConfig:%s) (mount-id:testFS-123)", common.GetVersion(), expectedConfigProto))
	assert.Equal(t.T(), expectedUserAgent, userAgent)
}
...
func (t *MainTest) TestGetUserAgentWithConfig_NilConfig() {
	...
	expectedUserAgent := strings.TrimSpace(fmt.Sprintf("gcsfuse/%s (GPN:gcsfuse-AppName) (mount-id:testFS-123)", common.GetVersion()))
	assert.Equal(t.T(), expectedUserAgent, userAgent)
	assert.Equal(t.T(), getUserAgent("AppName", "testFS-123"), userAgent)
}

func (t *MainTest) TestGetUserAgentWithConfig_NoAppNameAndNoEnvVar() {
	mountConfig := &cfg.Config{}

	userAgent := getUserAgentWithConfig("", mountConfig, "testFS-123")

	expectedConfigProto, err := mountConfig.SerializeConfigToProtoBase64()
	require.NoError(t.T(), err)
	expectedUserAgent := strings.TrimSpace(fmt.Sprintf("gcsfuse/%s (GPN:gcsfuse) (GCSFuseConfig:%s) (mount-id:testFS-123)", common.GetVersion(), expectedConfigProto))
	assert.Equal(t.T(), expectedUserAgent, userAgent)
}
