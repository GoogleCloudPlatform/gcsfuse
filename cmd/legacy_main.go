...
func mountWithArgs(bucketName string, mountPoint string, newConfig *cfg.Config, metricHandle metrics.MetricHandle, traceHandle tracing.TraceHandle, viperConfig *viper.Viper) (mfs *fuse.MountedFileSystem, err error) {
	...
	if bucketName != canned.FakeBucketName {
		userAgent := getUserAgent(newConfig.AppName, getConfigForUserAgent(newConfig), logger.MountInstanceID(fsName(bucketName, newConfig.OnlyDir), newConfig.MountId))
...
func fsName(bucketName, onlyDir string) string {
...
func Mount(mountInfo *mountInfo, bucketName, mountPoint string) (err error) {
	...
	logger.UpdateDefaultLogger(newConfig.Logging.Format, fsName(bucketName, newConfig.OnlyDir), newConfig.MountId)

	if newConfig.Foreground {
		err = logger.InitLogFile(newConfig.Logging, fsName(bucketName, newConfig.OnlyDir), newConfig.MountId)
		if err != nil {
	...
	if cfg.IsMetricsEnabled(&newConfig.Metrics) {
		metricExporterShutdownFn = monitor.SetupOTelMetricExporters(ctx, newConfig, logger.MountInstanceID(fsName(bucketName, newConfig.OnlyDir), newConfig.MountId))
		if metricHandle, err = metrics.NewOTelMetrics(ctx, int(newConfig.Metrics.Workers), int(newConfig.Metrics.BufferSize)); err != nil {
			metricHandle = metrics.NewNoopMetrics()
		}
	}
	shutdownTracingFn := monitor.SetupTracing(ctx, newConfig, logger.MountInstanceID(fsName(bucketName, newConfig.OnlyDir), newConfig.MountId))
