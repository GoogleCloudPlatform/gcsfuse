...
func forwardedEnvVars() []string {
	...
	// mount instance Id to identify logs from different mounts.
	env = append(env, fmt.Sprintf("%s=%s", logger.GCSFuseMountInstanceIdEnvKey, logger.MountInstanceId))
	return env
}

func Mount(newConfig *cfg.Config, wrapperConfig map[string]interface{}, bucketName, mountPoint string) (err error) {
	// Ideally this call to SetLogFormatAndFsName (which internally creates a new defaultLogger)
	...
	logger.SetLogFormatAndFsName(newConfig.Logging.Format, fsName(bucketName))
