...
func getUserAgentWithConfig(appName, config string, mountConfig *cfg.Config, mountInstanceID string) string {
	...
	if mountConfig != nil {
		var err error
		fullConfig, err = cfg.SerializeConfigToGzippedJSONBase64(mountConfig)
		if err != nil {
			fullConfig = ""
		}
	}
	...
	gcsfuseMetadataImageType := os.Getenv("GCSFUSE_METADATA_IMAGE_TYPE")
	if len(gcsfuseMetadataImageType) > 0 {
		userAgent = fmt.Sprintf("gcsfuse/%s %s (GPN:gcsfuse-%s) (Cfg:%s) (CfgJson:%s)", common.GetVersion(), appName, gcsfuseMetadataImageType, config, fullConfig)
		userAgent = strings.Join(strings.Fields(userAgent), " ")
	} else if len(appName) > 0 {
		userAgent = fmt.Sprintf("gcsfuse/%s (GPN:gcsfuse-%s) (Cfg:%s) (CfgJson:%s)", common.GetVersion(), appName, config, fullConfig)
	} else {
		userAgent = fmt.Sprintf("gcsfuse/%s (GPN:gcsfuse) (Cfg:%s) (CfgJson:%s)", common.GetVersion(), config, fullConfig)
	}
	return fmt.Sprintf("%s (mount-id:%s)", userAgent, mountInstanceID)
}
...
// Deprecated: getConfigForUserAgent returns minimum configuration details created in a bitset fashion.
// This 6-bit configuration representation is deprecated and will be replaced by the gzipped JSON configuration (CfgJson).
