...
func TestArgsParsing_FileSystemFlags(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "normal",
			args: []string{"gcsfuse", "--dir-mode=0777", "--disable-parallel-dirops", "--experimental-enable-entry-timeout", "--experimental-enable-readdirplus", "--file-mode=0666", "--o", "ro", "--gid=7", "--ignore-interrupts=false", "--kernel-list-cache-ttl-secs=300", "--rename-dir-limit=10", "--temp-dir=~/temp", "--uid=8", "--precondition-errors=false", "abc", "pqr"},
			expectedConfig: &cfg.Config{
				FileSystem: cfg.FileSystemConfig{
					DirMode:                        0777,
					DisableParallelDirops:          true,
					ExperimentalEnableEntryTimeout: true,
		...
		},
		{
			...
			expectedConfig: &cfg.Config{
				FileSystem: cfg.FileSystemConfig{
					...
					ExperimentalEnableEntryTimeout: false,
...
func TestArgsParsing_FileSystemFlagsThrowsError(t *testing.T) {
	tests := []struct {
		...
		{
			name: "invalid_experimental_enable_entry_timeout",
			args: []string{"gcsfuse", "--experimental-enable-entry-timeout=abc", "abc", "pqr"},
		},
	}
