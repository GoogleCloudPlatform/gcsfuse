...
func TestArgsParsing_GCSConnectionFlags(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			args: []string{"gcsfuse", "--billing-project=abc", "--client-protocol=http2", "--custom-endpoint=www.abc.com", "--experimental-control-client-on-direct-path=false", "--experimental-enable-json-read", "--experimental-grpc-conn-pool-size=20", "--http-client-timeout=20s", "--limit-bytes-per-sec=30", "--limit-ops-per-sec=10", "--max-conns-per-host=1000", "--max-idle-conns-per-host=20", "--sequential-read-size-mb=70", "abc", "pqr"},
			expectedConfig: &cfg.Config{
				GcsConnection: cfg.GcsConnectionConfig{
					BillingProject:                        "abc",
					ClientProtocol:                        "http2",
					CustomEndpoint:                        "www.abc.com",
					ExperimentalControlClientOnDirectPath: false,
		...
		},
		{
			...
			expectedConfig: &cfg.Config{
				GcsConnection: cfg.GcsConnectionConfig{
					...
					CustomEndpoint:                        "",
					ExperimentalControlClientOnDirectPath: true,
