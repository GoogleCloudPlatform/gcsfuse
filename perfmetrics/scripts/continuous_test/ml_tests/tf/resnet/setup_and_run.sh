...
gcloud compute instances delete --quiet tf-resnet-7d
...
gcloud compute ssh tf-resnet-7d --zone us-central1-c --command "mkdir github; cd github; git clone https://github.com/GoogleCloudPlatform/gcsfuse.git; cd gcsfuse; git checkout ai_ml_test;" --internal-ip
gcloud compute ssh tf-resnet-7d --zone us-central1-c --command "cd github/perfmetrics/scripts/continuous_test/ml_tests/tf/resnet/; export KOKORO_ARTIFACTS_DIR=$HOME; bash build.sh 1> ~/a.out 2> ~/e.err &;" --internal-ip
sleep 100s
