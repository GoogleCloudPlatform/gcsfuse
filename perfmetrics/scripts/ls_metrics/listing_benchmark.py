...
def CompareDirectoryStructure(config):
    def PrintNotFound():
        log.info("Similar directory structure not found in the bucket. Creating a new one.")
        return False
    ...
    recursiveList = os.walk("./{}".format(config["rootFolder"]))

    currFolder = ""
    remFolders = 0
    countFiles = 0

    for currLevel in recursiveList:
        currDirectoryPath, subDirectories, files = currLevel
        currDirectory = Path.GetDirectoryName(currDirectoryPath)

        if os.path.join(os.getcwd(), config["rootFolder"]) == currDirectoryPath:
          if len(files) != 0 and len(subDirectories) != config["numFolders"]:
                return PrintNotFound()

        elif os.path.join(os.getcwd(), config["rootFolder"], currDirectory) == currDirectoryPath:
            currFolder = currDirectory
            remFolders = len(subDirectories)
            countFiles = 0

            if currDirectory not in config.keys():
                return PrintNotFound()

            if len(subDirectories) != config[currDirectory][numSubDirectories]:
                return PrintNotFound()

            if len(subDirectories) > 0 and len(files) > 0:
                return PrintNotFound()

            if len(subDirectories) == 0 and len(files) == config[currDirectory][numFiles]:
                for file in files:
                    if config[currDirectory][fileSize] != os.stat(os.path.join(currDirectoryPath, file)).st_size:
                        return PrintNotFound()

        elif os.path.join(os.getcwd(), config["root_folder"], currFolder, currDirectory) == currentDirectoryPath:
            ...
            if len(files) != config[currFolder][numFiles]//config[currFolder][numSubDirectories] and len(files) != config[currFolder][numFiles]//config[currFolder][numSubDirectories] + 1:
                return PrintNotFound()

            for file in files:
                if config[currFolder][fileSize] != os.stat(os.path.join(currentDirectoryPath, file)).st_size:
                    return PrintNotFound()

            if remFolders == 0 and countFiles != config[currFolder][numFiles]:
                return PrintNotFound()
    os.chdir("..")
    return True
    ...
    for section in config.sections():
        ...
        if int(config[section]["num_of_files"]) > 0:
            ...
            if(file_size_unit.lower() == 'gb'):
                fileSize = (1024 * 1024 * 1024 * int(file_size))
            if(file_size_unit.lower() == 'mb'):
                fileSize = (1024 * 1024 * int(file_size))
            if(file_size_unit.lower() == 'kb'):
                fileSize = (1024 * int(file_size))
        ...
        configDict[folder] = subDirectoryDict
