...
then
  python3 listing_benchmark.py config.json --gcsfuse_flags "$1" --command "ls -R" --num_samples 30 --message "Testing CT setup."
else
  python3 listing_benchmark.py config.json --gcsfuse_flags "$1" --command "ls -R" --num_samples 30 --upload --message "Testing CT setup."
fi