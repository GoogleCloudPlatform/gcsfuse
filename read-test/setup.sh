...
function mount() {
    ...
    # mkdir -p ~/bucket-grpc
    # gcsfuse --config-file ~/dev/gcsfuse-tools/read-test/config.yaml $TEST_BUCKET_GRPC "~/bucket-grpc"
    # cd ~/bucket-grpc mkdir ./200M

    
}


function mount_and_run_for_zonal() {
    export TEST_BUCKET_ZONAL=fastbyte-team-princer-zb-write-test-uw4a
    export TEST_BUCKET_GRPC=princer-grpc-write-test-uw4a

    mkdir -p ~/bucket-zonal
    gcsfuse --config-file ~/dev/gcsfuse-tools/read-test/config.yaml $TEST_BUCKET_ZONAL "~/bucket-zonal"
    cd ~/bucket-zonal && mkdir ./200M
...
function mount_and_run_for_regional() {
    export TEST_BUCKET_ZONAL=fastbyte-team-princer-zb-write-test-uw4a
    export TEST_BUCKET_GRPC=princer-grpc-write-test-uw4a
