...
func computeProtoMappings(params []Param) ([]protoMappingTemplateData, error) {
	mappings := make([]protoMappingTemplateData, 0, len(params))
	for _, p := range params {
		if p.ConfigPath == "" || p.ProtoFieldName == "" || p.ProtoType == "" {
			continue
		}
		...
		goAccess := "config." + strings.Join(goParts, ".")
		...
		var expr string
		if highRiskTextTypes[p.Type] && !whitelistedTextConfigPaths[p.ConfigPath] {
			switch p.Type {
			case "[]string":
