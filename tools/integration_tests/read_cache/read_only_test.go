...
func readMultipleFiles(numFiles int, ctx context.Context, storageClient *storage.Client, fileNames []string, fileSize int64, t *testing.T) (expectedOutcome []*Expected) {
	for i := 0; i < numFiles; i++ {
		expectedOutcome = append(expectedOutcome, readFileAndValidateCacheWithGCS(ctx, storageClient, fileNames[i], fileSize, true, chunkSizeToRead, offset, t))
	}
...
func (s *readOnlyTest) TestSecondSequentialReadIsCacheHit(t *testing.T) {
	...
	// Read file 1st time.
	expectedOutcome1 := readFileAndValidateCacheWithGCS(s.ctx, s.storageClient, testFileName, fileSize, true, chunkSizeToRead, offset, t)
	// Read file 2nd time.
	expectedOutcome2 := readFileAndValidateCacheWithGCS(s.ctx, s.storageClient, testFileName, fileSize, true, chunkSizeToRead, offset, t)
...
func (s *readOnlyTest) TestReadFileSequentiallyLargerThanCacheCapacity(t *testing.T) {
	...
	// Read file 1st time.
	expectedOutcome1 := readFileAndValidateFileIsNotCached(s.ctx, s.storageClient, largeFileName, true, offset, chunkSizeToRead, t)
	// Read file 2nd time.
	expectedOutcome2 := readFileAndValidateFileIsNotCached(s.ctx, s.storageClient, largeFileName, true, offset, chunkSizeToRead, t)
...
func (s *readOnlyTest) TestReadFileRandomlyLargerThanCacheCapacity(t *testing.T) {
	...
	expectedOutcome1 := readFileAndValidateFileIsNotCached(s.ctx, s.storageClient, largeFileName, false, chunkSizeToRead, randomReadOffset, t)
	// Read file sequentially again.
	expectedOutcome2 := readFileAndValidateFileIsNotCached(s.ctx, s.storageClient, largeFileName, true, chunkSizeToRead, offset, t)
