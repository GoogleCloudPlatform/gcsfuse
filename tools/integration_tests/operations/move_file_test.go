...
func (s *OperationSuite) TestMoveFileWithinSameDirectory() {
...
func (s *OperationSuite) TestMoveFileWithinDifferentDirectory() {
	testDir := setup.SetupTestDirectoryOnMntDir(s.mountConfiguration.MntDir(), TestDirName(s.T()))
	dirPath := path.Join(testDir, "Test")
	filePath := path.Join(dirPath, MoveFile)
...
func (s *OperationSuite) TestMoveFileWithDestFileExist() {
