...
type ParallelDiropsSuite struct {
	suite.Suite
...
func (s *ParallelDiropsSuite) createDirStructure() testDirStrucure {
	var tds testDirStrucure
	tds.testDir = setup.SetupTestDirectoryWithMntDir(s.runCfg.MntDir, DirForOperationTests+"-"+setup.GenerateRandomString(5))
...
func (s *ParallelDiropsSuite) TestParallelLookUpsForSameFile() {
...
func (s *ParallelDiropsSuite) TestParallelReadDirs() {
	// Create directory structure for testing.
	tds := s.createDirStructure()
	defer deleteDirStructure(tds)
...
func (s *ParallelDiropsSuite) TestParallelLookUpAndDeleteSameDir() {
	// Create directory structure for testing.
	tds := s.createDirStructure()
	defer deleteDirStructure(tds)
...
func (s *ParallelDiropsSuite) TestParallelLookUpsForDifferentFiles() {
	// Create directory structure for testing.
	tds := s.createDirStructure()
	defer deleteDirStructure(tds)
	var stat1, stat2 os.FileInfo
...
func (s *ParallelDiropsSuite) TestParallelReadDirAndMkdirInsideSameDir() {
	// Create directory structure for testing.
	tds := s.createDirStructure()
	defer deleteDirStructure(tds)
	readDirFunc := func(wg *sync.WaitGroup, dirPath string, dirEntries *[]os.DirEntry, err *error) {
...
func (s *ParallelDiropsSuite) TestParallelLookUpAndDeleteSameFile() {
	// Create directory structure for testing.
	tds := s.createDirStructure()
	defer deleteDirStructure(tds)
...
func (s *ParallelDiropsSuite) TestParallelLookUpAndRenameSameFile() {
	// Create directory structure for testing.
	tds := s.createDirStructure()
	defer deleteDirStructure(tds)
...
func (s *ParallelDiropsSuite) TestParallelLookUpAndMkdirSameDir() {
	// Create directory structure for testing.
	tds := s.createDirStructure()
	defer deleteDirStructure(tds)
	mkdirFunc := func(wg *sync.WaitGroup, dirPath string, err *error) {
