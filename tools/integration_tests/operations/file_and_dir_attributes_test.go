...
func (s *OperationSuite) checkIfObjectAttrIsCorrect(objName string, preCreateTime time.Time, postCreateTime time.Time, byteSize int64, t *testing.T) {
	oStat, err := os.Stat(objName)

	if err != nil {
		t.Errorf("os.Stat error: %s, %v", objName, err)
	}
	statObjName := path.Join(s.mountConfiguration.MntDir(), TestDirName(s.T()), oStat.Name())
...
func (s *OperationSuite) TestFileAttributes() {
	...
	s.checkIfObjectAttrIsCorrect(fileName, preCreateTime, postCreateTime, BytesWrittenInFile, s.T())
}

func (s *OperationSuite) TestEmptyDirAttributes() {
	testDir := setup.SetupTestDirectoryOnMntDir(s.mountConfiguration.MntDir(), TestDirName(s.T()))
	...
	s.checkIfObjectAttrIsCorrect(path.Join(testDir, DirAttrTest), preCreateTime, postCreateTime, 0, s.T())
}

func (s *OperationSuite) TestNonEmptyDirAttributes() {
	testDir := setup.SetupTestDirectoryOnMntDir(s.mountConfiguration.MntDir(), TestDirName(s.T()))
	...
	s.checkIfObjectAttrIsCorrect(dirName, preCreateTime, postCreateTime, 0, s.T())
}
