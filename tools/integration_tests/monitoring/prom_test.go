...
func TestMain(m *testing.M) {
	...
	if len(configFile.Monitoring) == 0 {
		...
		testEnv.cfg.Configs = make([]test_suite.ConfigItem, 3)
		// Default config for TestPromOTELSuite
		testEnv.cfg.Configs[0].Flags = []string{
			"--prometheus-port=9190 --cache-dir=/tmp/gcsfuse-cache --log-file=/gcsfuse-tmp/monitoring.log",
		}
		testEnv.cfg.Configs[0].Compatible = map[string]bool{"flat": true, "hns": true}
		testEnv.cfg.Configs[0].Run = "TestPromOTELSuite"
		// Default config for TestPromBufferedReadSuite
		...
		testEnv.cfg.Configs[1].Compatible = map[string]bool{"flat": true, "hns": true}
		testEnv.cfg.Configs[1].Run = "TestPromBufferedReadSuite"
		// Default config for TestPromGrpcMetricsSuite
		testEnv.cfg.Configs[2].Flags = []string{
			"--client-protocol=grpc --enable-grpc-metrics=true --prometheus-port=9192 --log-file=/gcsfuse-tmp/prom_grpc_metrics.log",
		}
		testEnv.cfg.Configs[2].Compatible = map[string]bool{"flat": true, "hns": true}
		testEnv.cfg.Configs[2].Run = "TestPromGrpcMetricsSuite"
	}
	testEnv.cfg = &configFile.Monitoring[0]
