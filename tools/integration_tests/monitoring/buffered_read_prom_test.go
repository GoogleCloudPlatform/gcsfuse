...
func (testSuite *PromBufferedReadTest) TestRandomReadFallback() {
	...
	assertNonZeroCountMetric(testSuite.T(), "buffered_read_fallback_trigger_count", "fallback_reason", "random_read_detected")
}
...
func (testSuite *PromBufferedReadTest) TestInsufficientMemoryFallback() {
	...
	require.NoError(testSuite.T(), err)
	assertNonZeroCountMetric(testSuite.T(), "buffered_read_fallback_trigger_count", "fallback_reason", "insufficient_memory")
}
