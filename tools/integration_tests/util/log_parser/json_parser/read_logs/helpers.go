...
// parseSparseDownloadLog parses a sparse download log message and adds details
// to the structuredLogs map.
func parseSparseDownloadLog(startTimeStampSec, startTimeStampNanos int64, logsMessage string, structuredLogs map[string]*Job) error {
	...
	if !ok {
		structuredLogs[jobID] = &Job{
			...
			ObjectName:      objectName,
			BucketName:      bucketName,
			SparseDownloads: []SparseDownloadLogEntry{entry},
		}
	} else {
		jobEntry.SparseDownloads = append(jobEntry.SparseDownloads, entry)
	}
	return nil
}
