...
func RunTestsForImplicitDirAndExplicitDir(config test_suite.TestConfig, flags [][]string, m *testing.M) int {
	if config.MountedDirectory != "" && config.TestBucket != "" {
		setup.RunTestsForMountedDirectoryWithConfigFile(config.MountedDirectory, m)
	}
	...
	setup.SetUpTestDirForTestBucketFlagWithConfigFile()
