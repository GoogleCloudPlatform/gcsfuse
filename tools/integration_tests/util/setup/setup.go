...
// CheckAndPopulateCacheDir checks for the --cache-dir flag in the test
// configurations. If the test is for a mounted directory, it ensures that a
// non-empty cache directory is provided and fails if it's not. For test
// bucket-based tests, it automatically populates the cache directory path if
// it's missing or empty.
func CheckAndPopulateCacheDir(testConfigs *[]test_suite.TestConfig, testName string) {
	if len(*testConfigs) == 0 {
		return
	}
	// For mounted directory, check if a non-empty cache-dir is present.
	if (*testConfigs)[0].MountedDirectory != "" {
		for i := range (*testConfigs)[0].Configs {
			configItem := &(*testConfigs)[0].Configs[i]
			for _, flagStr := range configItem.Flags {
				if strings.Contains(flagStr, "file-cache-max-size-mb") && !hasNonEmptyCacheDir(flagStr) {
					log.Fatalf("A non-empty cache directory must be provided for mounted directory tests in %s.", testName)
				}
			}
		}
	} else { // For test bucket, populate cache-dir if missing or empty.
		for i := range (*testConfigs)[0].Configs {
			configItem := &(*testConfigs)[0].Configs[i]
			for j, flagStr := range configItem.Flags {
				if strings.Contains(flagStr, "file-cache-max-size-mb") && !hasNonEmptyCacheDir(flagStr) {
					// Since hasNonEmptyCacheDir is false, any existing --cache-dir flag is empty.
					// We remove it by rebuilding the flag string and then append a new one with a valid path.
					var newFields []string
					for _, field := range strings.Fields(flagStr) {
						if !strings.HasPrefix(field, "--cache-dir=") {
							newFields = append(newFields, field)
						}
					}
					newFlagStr := strings.Join(newFields, " ")
					(*testConfigs)[0].Configs[i].Flags[j] = newFlagStr + fmt.Sprintf(" --cache-dir=%s/cache-dir-%s-%s", os.TempDir(), testName, GenerateRandomString(4))
				}
			}
		}
	}
...
func hasNonEmptyCacheDir(flagStr string) bool {
	for _, field := range strings.Fields(flagStr) {
		if strings.HasPrefix(field, "--cache-dir=") {
			if len(strings.TrimPrefix(field, "--cache-dir=")) > 0 {
				return true
			}
		}
	}
