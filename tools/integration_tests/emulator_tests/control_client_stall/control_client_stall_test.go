...
func (c *controlClientStall) SetupTest() {
	c.configPath = "../configs/control_client_stall_2s.yaml"
...
// os.MkdirAll completes successfully even when a 2-second stall is induced, 
...
func (c *controlClientStall) TestCreateFolderStallInducedShouldComplete() {
	...
	assert.GreaterOrEqual(c.T(), elapsedTime, forcedStallTime)
...
func TestControlClientStall(t *testing.T) {
	...
	for _, flags := range flagsSet {
		ts.flags = flags
		log.Printf("Running Control Client Stall tests with flags: %s", ts.flags)
		suite.Run(t, ts)
	}
}