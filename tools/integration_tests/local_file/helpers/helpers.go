...
func CreateLocalFile(fileName string, t *testing.T) (filePath string, f *os.File) {
	...
	if err != nil {
		t.Errorf("CreateLocalFile(%s): %v", fileName, err)
	}
...
func CloseLocalFile(f *os.File, fileName string, t *testing.T) {
	err := f.Close()
	if err != nil {
		t.Errorf("%s.Close(): %v", fileName, err)
	}
}
...
func ValidateObjectContents(fileName string, expectedContent string, t *testing.T) {
	...
	if expectedContent != gotContent {
		t.Errorf("GCS file %s content mismatch. Got: %s, Expected: %s ", fileName, gotContent, expectedContent)
	}
}
...
func ValidateNoFileOrDirError(filename string, t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "no such file or directory") {
		t.Errorf("os.Stat on unlinked local file. Expected: %s, Got: %v",
...
func ReadDirectory(dirPath string, t *testing.T) (entries []os.DirEntry) {
	...
	if err != nil {
		t.Errorf("os.ReadDir(%s) err: %v", dirPath, err)
	}
	return
}
...
func VerifyLocalFileEntry(entry os.DirEntry, fileName string, size int64, t *testing.T) {
	if entry.IsDir() {
		t.Errorf("Expected: file entry, Got: directory entry.")
	}
	if entry.Name() != fileName {
		t.Errorf("File name, Expected: %s, Got: %s", fileName, entry.Name())
	}
	fileInfo, err := entry.Info()
	if err != nil {
		t.Errorf("%s.Info() err: %v", fileName, err)
	}
	if fileInfo.Size() != size {
		t.Errorf("Local file %s size, Expected: %d, Got: %d", fileName, size, fileInfo.Size())
	}
}

func VerifyDirectoryEntry(entry os.DirEntry, dirName string, t *testing.T) {
	if !entry.IsDir() {
		t.Errorf("Expected: directory entry, Got: file entry.")
	}
	if entry.Name() != dirName {
		t.Errorf("File name, Expected: %s, Got: %s", dirName, entry.Name())
	}
}
...
func UnlinkShouldNotThrowError(filePath string, t *testing.T) {
	...
	if err != nil {
		t.Errorf("os.Remove(%s): %v", filePath, err)
	}
}
...
func SyncOnLocalFileShouldNotThrowError(fh *os.File, fileName string, t *testing.T) {
	...
	if err != nil {
		t.Errorf("%s.Sync(): %v", fileName, err)
	}
}
...
func CreateExplicitDirShouldNotThrowError(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Error while creating directory, err: %v", err)
	}
}
...
func RemoveDirShouldNotThrowError(dirName string, t *testing.T) {
	...
	if err != nil {
		t.Errorf("os.RemoveAll(%s): %v", dirPath, err)
	}
}
...
func SymLinkShouldNotThrowError(filePath, symlinkName string, t *testing.T) {
	...
	if err != nil {
		t.Errorf("os.Symlink(%s, %s): %v", filePath, symlinkName, err)
	}
}
...
func VerifyReadLink(filePath, symlinkName string, t *testing.T) {
	...
	if err != nil {
		t.Errorf("os.Readlink(%s): %v", symlinkName, err)
	}
	if filePath != target {
		t.Errorf("Symlink target mismatch. Expected: %s, Got: %s", filePath, target)
	}
}
...
func VerifyReadFile(symlinkName string, t *testing.T) {
	...
	if err != nil {
		t.Errorf("os.ReadFile(%s): %v", symlinkName, err)
	}
	if FileContents != string(contents) {
		t.Errorf("Symlink content mismatch. Expected: %s, Got: %s", FileContents, contents)
	}
}

func VerifyCountOfEntries(expected, got int, t *testing.T) {
	if expected != got {
		t.Errorf("entry count mismatch, expected: %d, got: %d", expected, got)
	}
}
...
func VerifyRenameOperationNotSupported(err error, t *testing.T) {
	if err == nil || !strings.Contains(err.Error(), "operation not supported") {
		t.Errorf("os.Rename(), expected err: %s, got err: %v",
