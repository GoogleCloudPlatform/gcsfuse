...
func TestMain(m *testing.M) {
	...
	if len(cfg.StreamingWrites) == 0 {
		...
		cfg.StreamingWrites[0].MountedDirectory = setup.MountedDirectory()
	...
	if cfg.StreamingWrites[0].MountedDirectory != "" && cfg.StreamingWrites[0].TestBucket != "" {
		os.Exit(setup.RunTestsForMountedDirectory(cfg.StreamingWrites[0].MountedDirectory, m))
	}
