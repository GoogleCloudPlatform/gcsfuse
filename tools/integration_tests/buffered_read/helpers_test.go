...
func readAndValidateChunk(f *os.File, offset, size int64, testDir, fileName string, t *testing.T) {
	t.Helper()
	content := make([]byte, size)
	n, err := f.ReadAt(content, offset)
	require.NoError(t, err)
	require.Equal(t, int(size), n)
	client.ValidateObjectChunkFromGCS(ctx, storageClient, testDir, fileName, offset, size, string(content), t)
}
