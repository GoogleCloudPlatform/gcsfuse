...
usage() {
  ...
  echo "    --run-package                 <regexes>      Comma-separated regexes for RB and ZB packages to run. Supports '!' prefix for exclusion."
...
# Args: $1 = name of the array variable.
shuffle() {
  local -n arr=$1
  local i j tmp
...
shuffle TEST_PACKAGES_FOR_RB

# Test packages for zonal buckets.
TEST_PACKAGES_FOR_ZB=("${TEST_PACKAGES_COMMON[@]}" "rapid_appends" "unfinalized_object")
shuffle TEST_PACKAGES_FOR_ZB
# Test packages for TPC buckets.
TEST_PACKAGES_FOR_TPC=("operations")

# Parse and apply --run-package filters if provided
if [[ -n "$RUN_PACKAGE_REGEXES" ]]; then
  IFS=',' read -r RB_REGEX ZB_REGEX <<< "$RUN_PACKAGE_REGEXES"
  
  if [[ -n "$RB_REGEX" ]]; then
    filter_array TEST_PACKAGES_FOR_RB "$RB_REGEX"
  fi
  
  if [[ -n "$ZB_REGEX" ]]; then
    filter_array TEST_PACKAGES_FOR_ZB "$ZB_REGEX"
  fi
fi
