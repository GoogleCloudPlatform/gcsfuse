...
func (t *BaseSuite) getCachedChunkCount() int {
	...
	err := filepath.WalkDir(t.sharedCacheDir, func(filePath string, d fs.DirEntry, err error) error {
		if err != nil {
			return nil // Ignore errors like directory not existing
		}
...
func (t *SharedChunkCacheTestSuite) TestCacheMiss() {
	...
	// Create a test file on GCS via the primary mount
	...
	// Read 2MB from the 2nd chunk - this will download and cache the entire 10MB chunk
	...
	// Verify that the entire 2nd chunk was cached (even though we only read 2MB)
	// The chunk cache downloads the entire 10MB chunk when any part is accessed
...
func (t *SharedChunkCacheTestSuite) TestCacheHit() {
	...
	// Read 2MB from the 2nd chunk via primary mount - this will cache the entire 10MB chunk
	primaryFilePath := path.Join(t.primaryMount.testDirPath, testFileName)
	...
	// Verify that the entire 2nd chunk was cached
	cachedChunkCount := t.getCachedChunkCount()
	require.Equal(t.T(), 1, cachedChunkCount, "Cache should contain exactly 1 chunk after reading from 2nd chunk")
	...
	// Capture modification times of all cache files before the second read
	...
	// Now read the same 2MB from the secondary mount - this should be served from cache
	...
	// Validate that both mounts read the same chunk content
	require.Equal(t.T(), primaryChunk, secondaryChunk, "Chunk content from both mounts should match")

	// Verify cache count didn't change since we're hitting the cache
	...
	// Verify that cache files were NOT modified (proving they weren't re-downloaded)
...
func (t *SharedChunkCacheTestSuite) TestCacheHitSingleMount() {
	...
	// First read: cache miss - downloads and caches the entire 10MB chunk
	primaryFilePath := path.Join(t.primaryMount.testDirPath, testFileName)
	...
	// Verify that the chunk was cached
	cachedChunkCount := t.getCachedChunkCount()
	...
	// Capture modification times of cache files before second read
	cacheFileTimes := t.getCacheFileModTimes()
	require.NotEmpty(t.T(), cacheFileTimes, "Should have cache files to track")

	// Second read: cache hit - should be served from cache
