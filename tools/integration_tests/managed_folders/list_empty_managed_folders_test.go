...
func (s *enableEmptyManagedFoldersTrue) Teardown(t *testing.T) {
	...
	_, testDir := setup.GetBucketAndObjectBasedOnTypeOfMount(TestDirForEmptyManagedFoldersTest)
	...
	setup.CleanupDirectoryOnGCS(ctx, storageClient, TestDirForEmptyManagedFoldersTest)
}
