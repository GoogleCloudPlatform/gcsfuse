...
# Uninstalls the custom Cloud Storage FUSE CSI driver from a target GKE cluster.
# Usage: tools/scripts/csi/uninstall_custom.sh [CLUSTER_PROJECT] [CLUSTER_NAME] [CLUSTER_LOCATION] [CSI_VERSION]
...
if [[ -z "${CLUSTER_PROJECT}" || -z "${CLUSTER_NAME}" || -z "${CLUSTER_LOCATION}" ]]; then
  ...
  echo "  or: make uninstall-custom-csi CLUSTER_PROJECT=<PROJECT> CLUSTER_NAME=<CLUSTER_NAME> CLUSTER_LOCATION=<LOCATION>"
  exit 1
fi

echo "--------------------------------------"
echo "Uninstalling Custom CSI Driver from the cluster"
...
if [[ "${MANAGED_ADDON}" == "True" ]]; then
  ...
  echo "To uninstall the managed add-on, run: make uninstall-managed-csi"
  exit 1
fi

if ! kubectl get csidriver gcsfuse.csi.storage.gke.io >/dev/null 2>&1; then
  echo "Cloud Storage FUSE custom CSI driver is not installed on cluster '${CLUSTER_NAME}'. Nothing to uninstall."
  exit 0
fi

# Create temporary directory and ensure cleanup on exit
TMP_DIR=$(mktemp -d /tmp/make-uninstall-custom-csi.XXXXXXXXXX)
...
echo "Custom CSI driver uninstallation completed."
echo ""
echo "To install the GKE managed CSI driver, run: make install-managed-csi"
