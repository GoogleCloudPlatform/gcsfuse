...
elif `grep -iq suse /etc/os-release` || grep -q rhel details.txt || grep -q centos details.txt;
then
...
SKIP_TEST_DIR_NON_PARALLEL_SUSE_ARM64=(
  "managed_folders"
  "list_large_dir"
)
...
function run_non_parallel_tests() {
  ...
  do
    skip_test = containsElement SKIP_TEST_DIR_NON_PARALLEL_SUSE_ARM64 $test_dir_np
    if [ $skip_test == 1 ]; then
      continue
    fi
