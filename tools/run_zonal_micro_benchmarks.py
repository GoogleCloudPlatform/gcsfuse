...
def extract_metrics(job, rw_type):
    ...
    # Use clat_ns (completion latency) to match benchmarks.md
...
def main():
    ...
    # 1c. Small-Block Random Reads (Reuses the 192x10 100M files)
    rand_read_100m = [
        {"bs": "4k",  "iodepth": 1, "label": "Random Read 100 MB (BS=4K, QD=1, 192x10)"},
        {"bs": "4k",  "iodepth": 4, "label": "Random Read 100 MB (BS=4K, QD=4, 192x10)"},
        {"bs": "64k", "iodepth": 1, "label": "Random Read 100 MB (BS=64K, QD=1, 192x10)"},
        {"bs": "64k", "iodepth": 4, "label": "Random Read 100 MB (BS=64K, QD=4, 192x10)"},
    ]
    for item in rand_read_100m:
        drop_system_caches()
        env_vars_rand = {
            "DIR": dir_100m, "FILESIZE": "100M", "BS": item["bs"],
            "NUMJOBS": "192", "NRFILES": "10", "IODEPTH": str(item["iodepth"]),
        }
    ...
    # =========================================================================
    # 5. RANDOM READ WORKLOADS
    # =========================================================================
    print("\n" + "=" * 50, flush=True)
    print("RANDOM READ WORKLOADS", flush=True)
    print("=" * 50, flush=True)

    # 5. Large-block Random Reads on 1G dataset (144 jobs x 10 files)
    print("\n--- Large-Block Random Reads (1GB dataset: 144 jobs x 10 files) ---", flush=True)
    os.makedirs(dir_1g, exist_ok=True)
    job_1g_w2 = run_fio(FIO_WRITE_TEMPLATE, env_vars_1g, "write_1g_rand.fio")
    ...
    for item in rand_read_1g:
        drop_system_caches()
        env_vars_rand = {
            "DIR": dir_1g, "FILESIZE": "1G", "BS": item["bs"],
            "NUMJOBS": "144", "NRFILES": "10", "IODEPTH": str(item["iodepth"]),
        }
