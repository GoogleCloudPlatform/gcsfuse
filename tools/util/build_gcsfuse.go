...
func BuildGcsfuse(dstDir string) (err error) {
...

	{
		...
		if os.Getenv("GOCOVERDIR") != "" {
			args = append(args, "--", "-cover", "-covermode=atomic", "-coverpkg=github.com/googlecloudplatform/gcsfuse/v3/...")
		}
		cmd := exec.Command(
			toolPath,
			args...,
...
func buildBuildGcsfuse(dst string) (err error) {
	...
	var srcDir string
	{
		var pkg *build.Package
		pkg, err = build.Import(
			"github.com/googlecloudplatform/gcsfuse/v3/tools/build_gcsfuse",
			"",
			build.FindOnly)
