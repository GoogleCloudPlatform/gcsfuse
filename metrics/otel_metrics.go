...
var (
	...
	ioDepthFsOpTypeReadAttrSet = metric.WithAttributeSet(attribute.NewSet(attribute.String("op_type", "read"),))
	ioDepthFsOpTypeWriteAttrSet = metric.WithAttributeSet(attribute.NewSet(attribute.String("op_type", "write"),))
...
type otelMetrics struct {
	...
	ioDepthFsOpTypeReadAtomic *atomic.Int64
	ioDepthFsOpTypeWriteAtomic *atomic.Int64
...
func (o *otelMetrics) IoDepthFs(
		inc int64, opType OpType) {
		switch opType {
		case OpTypeReadAttr:
		o.ioDepthFsOpTypeReadAtomic.Add(inc)
		case OpTypeWriteAttr:
		o.ioDepthFsOpTypeWriteAtomic.Add(inc)
		default:
			updateUnrecognizedAttribute(string(opType))
			return
	}
}
...
func NewOTelMetrics(ctx context.Context, workers int, bufferSize int) (*otelMetrics, error) {
  ...
  meter := otel.Meter("gcsfuse")
	...
	_, err15 := meter.Int64ObservableUpDownCounter("io/depth_fs",
		metric.WithDescription("Current number of concurrent I/O operations in the filesystem by operation type."),
		metric.WithUnit("{operations}"),
		metric.WithInt64Callback(func(_ context.Context, obsrv metric.Int64Observer) error {
			observeUpDownCounter(obsrv, &ioDepthFsOpTypeReadAtomic, ioDepthFsOpTypeReadAttrSet)
			observeUpDownCounter(obsrv, &ioDepthFsOpTypeWriteAtomic, ioDepthFsOpTypeWriteAttrSet)
			return nil
		}))
	...
	return &otelMetrics{
		...
		wg: &wg,
			...
			ioDepthFsOpTypeReadAtomic: &ioDepthFsOpTypeReadAtomic,
			ioDepthFsOpTypeWriteAtomic: &ioDepthFsOpTypeWriteAtomic,
