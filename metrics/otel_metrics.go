...
var (
	...
	bufferedReadBytesCountOperationTypeDownloadedAttrSet                                = metric.WithAttributeSet(attribute.NewSet(attribute.String("operation_type", "downloaded")))
	bufferedReadBytesCountOperationTypeReadAttrSet                                      = metric.WithAttributeSet(attribute.NewSet(attribute.String("operation_type", "read")))
...
func NewOTelMetrics(ctx context.Context, workers int, bufferSize int) (*otelMetrics, error) {
	...
	_, err0 := meter.Int64ObservableCounter("buffered_read/bytes_count",
		metric.WithDescription("The cumulative number of bytes read from or downloaded by the buffered reader."),
		metric.WithUnit("By"),
		metric.WithInt64Callback(func(_ context.Context, obsrv metric.Int64Observer) error {
			conditionallyObserve(obsrv, &bufferedReadBytesCountOperationTypeDownloadedAtomic, bufferedReadBytesCountOperationTypeDownloadedAttrSet)
			conditionallyObserve(obsrv, &bufferedReadBytesCountOperationTypeReadAtomic, bufferedReadBytesCountOperationTypeReadAttrSet)
			return nil
		}))
	...
	errs := []error{err0, err1, err2, err3, err4, err5, err6, err7, err8, err9, err10, err11, err12, err13, err14, err15}
