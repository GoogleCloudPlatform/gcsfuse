...
func GetSizeOnDiskAndClean(dirPath string, includesFilesInSize bool, deleteEmptyDirs bool) uint64 {
	size, _ := getSizeOnDiskAndClean(dirPath, includesFilesInSize, deleteEmptyDirs, nil)
	return size
}

// GetSizeOnDiskAndClean calculates the size of the directory and optionally removes empty directories.
// It combines the functionality of GetSizeOnDisk (with ignoreErrors=true) and RemoveEmptyDirs.
func GetSizeOnDiskAndCleanWithLocker(dirPath string, includesFilesInSize bool, deleteEmptyDirs bool, locker DirLocker) uint64 {
	size, _ := getSizeOnDiskAndClean(dirPath, includesFilesInSize, deleteEmptyDirs, locker)
	return size
}

func getSizeOnDiskAndClean(dirPath string, includesFilesInSize bool, deleteEmptyDirs bool, locker DirLocker) (uint64, bool) {
	var sizeOnDisk int64
	// Add the size of the root directory itself.
	info, err := os.Stat(dirPath)
	if err == nil {
	...
	for _, entry := range entries {
		if entry.IsDir() {
			...
			subDirSize, subDirEmpty := getSizeOnDiskAndClean(subDir, includesFilesInSize, deleteEmptyDirs, locker)

			// Attempt removal only if requested AND the subdirectory is effectively empty.
			if deleteEmptyDirs && subDirEmpty {
				// If the child directory is empty (or became empty), attempt to remove it.
				if locker != nil {
					locker.WriteLock(subDir)
					err = os.Remove(subDir)
					locker.WriteUnlock(subDir)
				} else {
					err = os.Remove(subDir)
				}
				if err != nil {
