...
var (
	...
	readRequestLatency = stats.Float64("fs/read_request_latency", "The latency of a file system read request.", stats.UnitMilliseconds)
	readRequestError   = stats.Int64("fs/read_request_error", "The number of errors generated by file system read operation.", stats.UnitDimensionless)
)
...
func init() {
	...
	if err := view.Register(
		...
		&view.View{
			Name:        "fs/read_request_error",
			Measure:     readRequestError,
			Description: "The cumulative number of errors generated by file system read operation",
			Aggregation: view.Sum(),
			TagKeys:     []tag.Key{tags.FSReadError},
		},
		&view.View{
			Name:        "fs/read_request_latency",
			Measure:     readRequestLatency,
			Description: "The cumulative distribution of file system read request latencies",
...
// recordReadRequest records a file system read request latency and error count
func recordReadRequest(ctx context.Context, method string, start time.Time, readErr error) {

	// record file system read error count
	if readErr != nil {
		if err := stats.RecordWithTags(
			ctx,
			[]tag.Mutator{
				tag.Upsert(tags.FSReadError, method),
			},
			readRequestError.M(1),
		); err != nil {
			// error in recording the file system read error count
			errorLogger.Printf("Cannot record File system read request count: %v", err)
		}
	}

	// record file system read request latency
	...
	if err := stats.RecordWithTags(
		...
		readRequestLatency.M(readLatency),
	); err != nil {
		// error in recording the file system read request latency
		errorLogger.Printf("Cannot record File System read request latency: %v", err)
	}
}
...
func (fs *monitoring) ReadFile(
	...
	recordReadRequest(ctx, "ReadFile", startTime, err)
	return err
}
