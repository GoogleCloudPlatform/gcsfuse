...
var (
	...
	opLatency    = stats.Float64("fs/ops_latency", "The latency of a file system operation.", stats.UnitMilliseconds)
	opErrorCount = stats.Int64("fs/ops_error_count", "The number of errors generated by file system operation.", stats.UnitDimensionless)
)
...
// recordOp records file system operation count, operation error count and the operation latency
func recordOp(ctx context.Context, method string, start time.Time, fsErr error) {

	// recording the operation count
	...
	// recording the operation error count
	if fsErr != nil {
		if err := stats.RecordWithTags(
			ctx,
			[]tag.Mutator{
			...
			// error in recording the error count of the file system operation
			errorLogger.Printf("Cannot record error count of the file system operation: %v", err)
		}
	}

	// recording the operation latency
	...
	if err := stats.RecordWithTags(
		...
		opLatency.M(latencyMs),
	); err != nil {
		// error in recording the latency of the file system operation
