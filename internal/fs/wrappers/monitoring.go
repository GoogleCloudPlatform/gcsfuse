...
var (
	...
	testMetric, _ = meter.Int64Counter("fs/test_metric", metric.WithDescription("The test metric"))
...
func recordOp(ctx context.Context, method string, start time.Time, fsErr error) {
	...
	testMetric.Add(ctx, 1, metric.WithAttributes(attribute.String("fs_op", method)))
