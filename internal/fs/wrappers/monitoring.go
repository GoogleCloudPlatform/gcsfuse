...
var (
	opsCountOC      = stats.Int64("fs/ops_count", "The number of ops processed by the file system.", monitor.UnitDimensionless)
	opsLatencyOC    = stats.Int64("fs/ops_latency", "The latency of a file system operation.", monitor.UnitMicroseconds)
	opsErrorCountOC = stats.Int64("fs/ops_error_count", "The number of errors generated by file system operation.", monitor.UnitDimensionless)
)
...
func recordOp(ctx context.Context, method string, start time.Time, fsErr error) {
	...
	if err := stats.RecordWithTags(
		...
		opsLatencyOC.M(time.Since(start).Microseconds()),
	); err != nil {
		// Error in opLatency.
