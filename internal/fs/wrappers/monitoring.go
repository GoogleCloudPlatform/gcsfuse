...
func init() {
	...
	if err := view.Register(
		...
		&view.View{
			...
			Aggregation: view.Sum(),
			TagKeys:     []tag.Key{tags.FSOp, tags.FSError, tags.FSErrorType},
		},
		&view.View{
			Name:        "fs/ops_latency",
			Measure:     opsLatency,
...
// fsErrStrAndType maps an error to an error string and an error type.
// Uncommon errors are bucketed into groups to reduce the cardinality of the fs error to save the monitoring cost.
func fsErrStrAndType(err error) (errStr string, errType string) {
	if err == nil {
		return "", ""
	}
	...
	return errno.Error(), mapToType(errno)
}

// mapToType maps an error to an error-type.
...
func mapToType(errno syscall.Errno) string {
	switch errno {
	case syscall.ELNRNG,
		syscall.ENODEV,
		syscall.ENONET,
...
func recordOp(ctx context.Context, method string, start time.Time, fsErr error) {
	...
	if fsErr != nil {
		errStr, errType := fsErrStrAndType(fsErr)
		if err := stats.RecordWithTags(
			ctx,
			[]tag.Mutator{
				tag.Upsert(tags.FSOp, method),
				tag.Upsert(tags.FSError, errStr),
				tag.Upsert(tags.FSErrorType, errType),
			},
			opsErrorCount.M(1),
		); err != nil {
