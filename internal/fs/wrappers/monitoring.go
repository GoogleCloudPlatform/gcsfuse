...
func init() {
	...
	if err := view.Register(
		...
		&view.View{
			...
			Aggregation: view.Sum(),
			TagKeys:     []tag.Key{tags.FSOp, tags.FSError, tags.FSErrorGroup},
		},
		&view.View{
			Name:        "fs/ops_latency",
			Measure:     opsLatency,
...
// fsErrStrAndGroup maps an error to an error string and an error group.
// Uncommon errors are bucketed into groups to reduce the cardinality of the fs error to save the monitoring cost.
func fsErrStrAndGroup(err error) (errStr string, errGrp string) {
	if err == nil {
		return "", ""
	}
	...
	return errno.Error(), mapToGroup(errno)
}

// mapToGroup maps an error to an error-group.
...
func mapToGroup(errno syscall.Errno) string {
	switch errno {
	case syscall.ELNRNG,
		syscall.ENODEV,
		syscall.ENONET,
...
func recordOp(ctx context.Context, method string, start time.Time, fsErr error) {
	...
	if fsErr != nil {
		errStr, errGrp := fsErrStrAndGroup(fsErr)
		if err := stats.RecordWithTags(
			ctx,
			[]tag.Mutator{
				tag.Upsert(tags.FSOp, method),
				tag.Upsert(tags.FSError, errStr),
				tag.Upsert(tags.FSErrorGroup, errGrp),
			},
			opsErrorCount.M(1),
		); err != nil {
