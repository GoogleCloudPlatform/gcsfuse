...
func (t *FileTest) SetUp(ti *TestInfo) {
	...
	t.backingObj, err = gcsutil2.CreateObject(
		t.ctx,
		t.bucket,
		fileName,
...
func (t *FileTest) WriteThenSync() {
	...
	statReq := &gcs2.StatObjectRequest{Name: t.in.Name().GcsObjectName()}
	...
	contents, err := gcsutil2.ReadObject(t.ctx, t.bucket, t.in.Name().GcsObjectName())
...
func (t *FileTest) Sync_Clobbered() {
	...
	newObj, err := gcsutil2.CreateObject(
		t.ctx,
		t.bucket,
