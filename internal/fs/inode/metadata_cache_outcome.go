...
// MetadataCacheOutcome records how the metadata cache answered a child lookup operation.
//
...
// hooks.
type MetadataCacheOutcome struct {
...
type metadataCacheOutcomeKey struct{}

// WithMetadataCacheOutcome returns a context carrying a fresh MetadataCacheOutcome, along with
// the outcome itself for the caller to read once the lookup has completed.
func WithMetadataCacheOutcome(ctx context.Context) (context.Context, *MetadataCacheOutcome) {
	outcome := &MetadataCacheOutcome{}
	return context.WithValue(ctx, metadataCacheOutcomeKey{}, outcome), outcome
}

// recordMetadataCacheOutcome stores how the cache answered on the outcome installed by
// WithMetadataCacheOutcome.
//
...
func recordMetadataCacheOutcome(ctx context.Context, cacheHit bool, entryStatus metrics.EntryStatus, lookupDetail metrics.LookupDetail) {
	if ctx == nil {
		return
	}
	outcome, ok := ctx.Value(metadataCacheOutcomeKey{}).(*MetadataCacheOutcome)
	if !ok || outcome == nil {
		return
	}
