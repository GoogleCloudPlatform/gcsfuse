...
func (t *OpenTest) NonExistent_CreateFlagNotSet() {
	...
	_, err = bucketutil.ReadObject(ctx, bucket, "foo")
...
func (t *OpenTest) NonExistent_CreateFlagSet() {
	...
	contents, err := bucketutil.ReadObject(ctx, bucket, "foo")
	AssertEq(nil, err)
...
func (t *DirectoryTest) ContentTypes() {
	...
	for _, name := range testCases {
		...
		o, err := bucket.StatObject(ctx, &bucket2.StatObjectRequest{Name: name})
		AssertEq(nil, err)
...
func (t *FileTest) UnlinkFile_NoLongerInBucket() {
	...
	AssertEq(
		...
		bucket.DeleteObject(
			ctx,
			&bucket2.DeleteObjectRequest{Name: "foo"}))
...
func (t *FileTest) Sync_NotDirty() {
	...
	// for it.
	statReq := &bucket2.StatObjectRequest{
		Name: "foo",
	}
...
func (t *SymlinkTest) CreateLink() {
	...
	o, err := bucket.StatObject(ctx, &bucket2.StatObjectRequest{Name: "bar"})
