...
func NewFileSystem(
	...
	fs := &fileSystem{
		...
		metaGenerationBackedInodes: make(map[inode.Name]inode.MetaGenerationFolderBackedInode),
...
type fileSystem struct {
	...
	metaGenerationBackedInodes map[inode.Name]inode.MetaGenerationFolderBackedInode
...
func (fs *fileSystem) mintInode(ic inode.Core) (in inode.Inode) {
	...
	case (ic.MinObject != nil || ic.Folders != nil) && ic.FullName.IsDir():
...
func (fs *fileSystem) lookUpOrCreateInodeIfNotStale(ic inode.Core) (in inode.Inode) {
	...
	if ic.Folders != nil {
		...
		for {
			...
			existingInode, ok := fs.metaGenerationBackedInodes[ic.FullName]

			// If we have no existing record, mint an inode and return it.
			if !ok {
				in = fs.mintInode(ic)
				fs.metaGenerationBackedInodes[in.Name()] = in.(inode.MetaGenerationFolderBackedInode)
			...
			if fs.metaGenerationBackedInodes[ic.FullName] != existingInode {
				existingInode.Unlock()
				continue
			}

			// Have we found the correct inode?
			cmp := folderMetaGeneration.Compare(existingInode.SourceMetaGenerationFolder())
			if cmp == 0 {
				in = existingInode
				return
			}
...
func (fs *fileSystem) lookUpOrCreateChildInode(
	...
	for n := 0; n < maxTries; n++ {
		...
		fmt.Println("Core in LookUpChild: ", core)
