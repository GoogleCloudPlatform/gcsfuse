...
func (fs *fileSystem) Rename(
	...
	if child.Bucket.BucketType() == gcs.Hierarchical {
		return fs.renameFileInHierarchicalBucket(ctx, oldParent, op.OldName, newParent, op.NewName)
	}
	return fs.renameFile(ctx, oldParent, op.OldName, child.MinObject, newParent, op.NewName)
}

func (fs *fileSystem) renameFileInHierarchicalBucket(ctx context.Context, oldParent inode.DirInode, oldName string, newParent inode.DirInode, newName string) error {
	oldParent.Lock()
	...
	_, err := oldParent.RenameFile(ctx, oldFileName.GcsObjectName(), newFileName.GcsObjectName())
