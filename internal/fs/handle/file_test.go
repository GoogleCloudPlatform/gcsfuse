...
func (t *fileTest) SetupTest() {
	...
	t.bucket = gcsx.NewSyncerBucket( /*appendThreshold=*/ 1 /*chunkRetryDeadlineSecs=*/, 120 /*chunkTransferTimeoutSecs=*/, 10, ".gcsfuse_tmp/", fake.NewFakeBucket(&t.clock, "some_bucket", gcs.BucketType{}))
}
...
func (t *fileTest) Test_Read_Success() {
	...
	fh := NewFileHandle(in, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), readMode, &cfg.Config{}, nil, nil, 0)
...
func (t *fileTest) Test_ReadWithReadManager_ErrorScenarios() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func() {
			...
			fh := NewFileHandle(testInode, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), readMode, &cfg.Config{}, nil, nil, 0)
			fh.inode.Lock()
...
func (t *fileTest) Test_ReadWithMrdKernelReader_Success() {
	...
	// Create File Inode.
	mockSyncerBucket := gcsx.NewSyncerBucket( /*appendThreshold=*/ 1 /*chunkRetryDeadlineSecs=*/, 120 /*chunkTransferTimeoutSecs=*/, 10, ".gcsfuse_tmp/", mockBucket)
...
func (t *fileTest) Test_ReadWithMrdKernelReader_NotAuthoritative() {
	// 1. Setup
	zonalBucket := gcsx.NewSyncerBucket( /*appendThreshold=*/ 1 /*chunkRetryDeadlineSecs=*/, 120 /*chunkTransferTimeoutSecs=*/, 10, ".gcsfuse_tmp/", fake.NewFakeBucket(&t.clock, "zonal_bucket", gcs.BucketType{Zonal: true}))
...
func (t *fileTest) Test_ReadWithMrdKernelReader_NilReader() {
	...
	nonZonalBucket := gcsx.NewSyncerBucket( /*appendThreshold=*/ 1 /*chunkRetryDeadlineSecs=*/, 120 /*chunkTransferTimeoutSecs=*/, 10, ".gcsfuse_tmp/", fake.NewFakeBucket(&t.clock, "non_zonal_bucket", gcs.BucketType{Zonal: true}))
...
func (t *fileTest) TestOpenMode() {
	...
	for _, tc := range testCases {
		...
		config := &cfg.Config{Write: cfg.WriteConfig{EnableStreamingWrites: false}}
		in := createFileInode(t.T(), &t.bucket, &t.clock, config, parent, "test_obj", nil, false)
		fh := NewFileHandle(in, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), tc.openMode, &cfg.Config{}, nil, nil, 0)
...
func (t *fileTest) TestFileHandle_Destroy_WithReaderAndReadManager() {
	...
	fh := NewFileHandle(fileInode, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), readMode, config, nil, nil, 0)
	fh.reader = mockReader
