...
func (t *fileTest) TestFileHandleWrite() {
	...
	fh := NewFileHandle(in, nil, false, nil, util.Write, &cfg.ReadConfig{}, nil, nil, nil)
	data := []byte("hello")
...
func (t *fileTest) Test_Read_Success() {
	...
	fh := NewFileHandle(in, nil, false, common.NewNoopMetrics(), util.Read, &cfg.ReadConfig{}, nil, nil, nil)
...
func (t *fileTest) Test_ReadWithReadManager_ErrorScenarios() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func() {
			...
			fh := NewFileHandle(testInode, nil, false, common.NewNoopMetrics(), util.Read, &cfg.ReadConfig{}, nil, nil, nil)
			fh.inode.Lock()
