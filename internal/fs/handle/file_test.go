...
func (t *fileTest) TestFileHandleWrite() {
	...
	fh := NewFileHandle(in, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), writeMode, &cfg.Config{}, nil, nil, nil, 0)
	data := []byte("hello")
...
func (t *fileTest) Test_IsValidReadManager_NilReadManager() {
	...
	fh := NewFileHandle(in, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), readMode, config, nil, nil, nil, 0)
	fh.inode.Lock()
...
func (t *fileTest) Test_Read_Success() {
	...
	fh := NewFileHandle(in, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), readMode, &cfg.Config{}, nil, nil, nil, 0)
...
func (t *fileTest) Test_ReadWithReadManager_ErrorScenarios() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func() {
			...
			fh := NewFileHandle(testInode, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), readMode, &cfg.Config{}, nil, nil, nil, 0)
			fh.inode.Lock()
...
func (t *fileTest) Test_ReadWithMrdKernelReader_Success() {
	...
	// Create File Handle.
	fh := NewFileHandle(in, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), readMode, &cfg.Config{FileSystem: cfg.FileSystemConfig{EnableKernelReader: true}}, nil, nil, nil, 0)
...
func (t *fileTest) Test_ReadWithMrdKernelReader_NilReader() {
	...
	// Create file handle.
	fh := NewFileHandle(in, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), readMode, &cfg.Config{FileSystem: cfg.FileSystemConfig{EnableKernelReader: false}}, nil, nil, nil, 0)
...
func (t *fileTest) TestOpenMode() {
	...
	for _, tc := range testCases {
		...
		config := &cfg.Config{Write: cfg.WriteConfig{EnableStreamingWrites: false}}
		in := createFileInode(t.T(), &t.bucket, &t.clock, config, parent, "test_obj", nil, false)
		fh := NewFileHandle(in, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), tc.openMode, &cfg.Config{}, nil, nil, nil, 0)
...
func (t *fileTest) TestFileHandle_Destroy_WithReaderAndReadManager() {
	...
	fh := NewFileHandle(fileInode, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), readMode, config, nil, nil, nil, 0)
	fh.reader = mockReader
...
func (t *fileTest) Test_ReadWithReadManager_FullReadSuccessWithBufferedRead() {
	...
	fh := NewFileHandle(in, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), readMode, config, workerPool, globalSemaphore, nil, 0)
	fh.inode.Lock()
...
func (t *fileTest) Test_ShouldSkipSizeChecks() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func() {
			...
			fh := NewFileHandle(in, nil, nil, false, metrics.NewNoopMetrics(), tracing.NewNoopTracer(), tc.openMode, config, nil, nil, nil, 0)
