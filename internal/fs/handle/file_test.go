...
func (t *fileTest) TestFileHandleWrite() {
	...
	fh := NewFileHandle(in, nil, false, nil, writeMode, &cfg.Config{}, nil, nil)
	data := []byte("hello")
...
func (t *fileTest) Test_IsValidReadManager_NilReadManager() {
	...
	fh := NewFileHandle(in, nil, false, nil, readMode, config, nil, nil)
	fh.inode.Lock()
...
func (t *fileTest) Test_Read_Success() {
	...
	fh := NewFileHandle(in, nil, false, metrics.NewNoopMetrics(), readMode, &cfg.Config{}, nil, nil)
...
func (t *fileTest) Test_ReadWithReadManager_ErrorScenarios() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func() {
			...
			fh := NewFileHandle(testInode, nil, false, metrics.NewNoopMetrics(), readMode, &cfg.Config{}, nil, nil)
			fh.inode.Lock()
...
func (t *fileTest) TestFileHandle_Destroy_WithReaderAndReadManager() {
	...
	fh := NewFileHandle(fileInode, nil, false, nil, readMode, config, nil, nil)
	fh.reader = mockReader
...
func (t *fileTest) Test_ReadWithReadManager_FullReadSuccessWithBufferedRead() {
	...
	fh := NewFileHandle(in, nil, false, metrics.NewNoopMetrics(), readMode, config, workerPool, globalSemaphore)
	fh.inode.Lock()
...
func (t *fileTest) Test_ReadWithReadManager_WorkloadInsightVisual() {
	...
	in.Lock()
	fh := NewFileHandle(in, nil, false, metrics.NewNoopMetrics(), readMode, config, nil, nil)
	in.Unlock()
