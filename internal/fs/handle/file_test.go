...
func (t *fileTest) SetupTest() {
	...
	t.bucket = gcsx.NewSyncerBucket(1, 120, 10, ".gcsfuse_tmp/", fake.NewFakeBucket(&t.clock, "some_bucket", gcs.BucketType{}))
}
...
func (t *fileTest) Test_ReadWithMrdKernelReader_Success() {
	...
	// Create File Inode.
	mockSyncerBucket := gcsx.NewSyncerBucket(1, 120, 10, ".gcsfuse_tmp/", mockBucket)
...
func (t *fileTest) Test_ReadWithMrdKernelReader_NotAuthoritative() {
	// 1. Setup
	zonalBucket := gcsx.NewSyncerBucket(1, 120, 10, ".gcsfuse_tmp/", fake.NewFakeBucket(&t.clock, "zonal_bucket", gcs.BucketType{Zonal: true}))
...
func (t *fileTest) Test_ReadWithMrdKernelReader_NilReader() {
	...
	nonZonalBucket := gcsx.NewSyncerBucket(1, 120, 10, ".gcsfuse_tmp/", fake.NewFakeBucket(&t.clock, "non_zonal_bucket", gcs.BucketType{Zonal: true}))
