...
func processArrivals(
	...
	for {
		...
		// Wait.
		err := throttle.WaitN(ctx, int(accumulated))
		if err != nil {
			return
		}
...
func (t *ThrottleTest) IntegrationTest() {
	...
	for i, tc := range testCases {
		// Create a throttle.
		capacity, err := ChooseLimiterCapacity(tc.limitRateHz,
			perCaseDuration)

		AssertEq(nil, err)

		throttle := rate.NewLimiter(rate.Limit(tc.limitRateHz), int(capacity))
