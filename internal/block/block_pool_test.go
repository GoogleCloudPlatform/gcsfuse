...
func (t *BlockPoolTest) TestInitBlockPool() {
	bp, err := NewPool(1024, 10, semaphore.NewWeighted(10), createBlock)
...
func (t *BlockPoolTest) TestInitBlockPoolForZeroBlockSize() {
	_, err := NewPool(0, 10, semaphore.NewWeighted(10), createBlock)
...
func (t *BlockPoolTest) TestInitBlockPoolForNegativeBlockSize() {
	_, err := NewPool(-1, 10, semaphore.NewWeighted(10), createBlock)
...
func (t *BlockPoolTest) TestInitBlockPoolForZeroMaxBlocks() {
	_, err := NewPool(10, 0, semaphore.NewWeighted(10), createBlock)
...
func (t *BlockPoolTest) TestInitBlockPoolForNegativeMaxBlocks() {
	_, err := NewPool(10, -1, semaphore.NewWeighted(10), createBlock)
...
func (t *BlockPoolTest) TestCreateBlockWithLargeSize() {
	...
	bp, err := NewPool(1024*1024*1024*1024, 10, semaphore.NewWeighted(10), createBlock)
	require.Nil(t.T(), err)
...
func (t *BlockPoolTest) TestClearFreeBlockChannel() {
	...
	for _, tt := range tests {
		t.Run(tt.name, func() {
			bp, err := NewPool(1024, 10, semaphore.NewWeighted(4), createBlock)
			require.Nil(t.T(), err)
			blocks := make([]Block, 4)
			for i := 0; i < 4; i++ {
...
func (t *BlockPoolTest) TestBlockPoolCreationAcquiresGlobalSem() {
	...
	bp, err := NewPool(1024, 3, globalBlocksSem, createBlock)
...
func (t *BlockPoolTest) TestClearFreeBlockChannelWithMultipleBlockPools() {
	...
	bp1, err := NewPool(1024, 3, globalMaxBlocksSem, createBlock)
	require.Nil(t.T(), err)
	bp2, err := NewPool(1024, 3, globalMaxBlocksSem, createBlock)
	require.Nil(t.T(), err)
...
func (t *BlockPoolTest) TestBlockPoolCreationFailsWhenGlobalMaxBlocksIsZero() {
	bp, err := NewPool(1024, 10, semaphore.NewWeighted(0), createBlock)
...
func (t *BlockPoolTest) TestGetWhenLimitedByGlobalBlocks() {
	bp, err := NewPool(1024, 10, semaphore.NewWeighted(2), createBlock)
	require.Nil(t.T(), err)
...
func (t *BlockPoolTest) validateGetBlockIsBlocked(bp *Pool[Block]) {
	t.T().Helper()
...
func (t *BlockPoolTest) validateGetBlockIsNotBlocked(bp *Pool[Block]) Block {
	t.T().Helper()
