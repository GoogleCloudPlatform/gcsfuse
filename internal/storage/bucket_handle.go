...
func withCreatePreconditions(obj *storage.ObjectHandle, req *gcs.CreateObjectRequest) (*storage.ObjectHandle, bool) {
	...
	return obj, preconditions.DoesNotExist
}
...
func (bh *bucketHandle) CreateObject(ctx context.Context, req *gcs.CreateObjectRequest) (o *gcs.Object, err error) {
	...
	sow, err := NewStreamedObjectWriter(ctx, obj, req, nil)
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		err = sow.Write(req.Contents)
		if err == io.EOF {
			logger.Debugf("Completed upload. totalSizeUploaded-so-far=%d, successfully-uploaded-in-last-upload=%d, chunk-size=%d for object %s: %v", sow.BytesWrittenSoFar(), sow.BytesWrittenSoFar(), sow.wc.ChunkSize, req.Name, err)
			break
		} else if err != nil {
			err = fmt.Errorf("failed to upload chunk after totalSizeUploaded-so-far=%d, successfully-uploaded=%d, upload-size=%d for object %s: %v", sow.BytesWrittenSoFar(), n, sow.wc.ChunkSize, req.Name, err)
			o, _ = sow.Close()
			return
		}
	}
...
func (bh *bucketHandle) CreateAsyncObjectWriter(
	ctx context.Context,
	req *gcs.CreateObjectRequest,
	progressFunc func(int64)) (sow gcs.ObjectWriter, err error) {
	obj := bh.bucket.Object(req.Name)
