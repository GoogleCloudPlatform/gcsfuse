...
func NewStorageHandle(ctx context.Context, clientConfig storageutil.StorageClientConfig, billingProject string) (sh StorageHandle, err error) {
	...
	if clientConfig.EnableHNS && !strings.Contains(clientConfig.CustomEndpoint, "/storage/v1") {
		...
		// rawStorageControlClient is initialized with the clean UserAgent and retained in
		// storageClient. It is utilized in BucketHandle to construct per-bucket controlClient
		// instances with folder retries for all HNS Folder APIs (CreateFolder, DeleteFolder,
		// RenameFolder, GetFolder), ensuring routine filesystem operations do not carry the
		// large GCSFuseConfig payload.
		//
		// rawStorageControlClientForStorageLayout is specifically used for the one-time
		// GetStorageLayout discovery call, attaching ConfigUserAgent with the config proto payload.
