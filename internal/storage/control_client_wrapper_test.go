...
func (t *StorageLayoutRetryWrapperTest) TestGetStorageLayout_SuccessOnFirstAttempt() {
	...
	client := newRetryWrapper(t.stallingClient, clientConfig, false)
...
func (t *StorageLayoutRetryWrapperTest) TestGetStorageLayout_AllAttemptsTimeOut() {
	...
	client := newRetryWrapper(t.stallingClient, clientConfig, false)
	client.(*storageControlClientWithRetry).retryConfig.RetryDeadline = 1000 * time.Microsecond
	client.(*storageControlClientWithRetry).retryConfig.TotalRetryBudget = 10000 * time.Microsecond
	req := &controlpb.GetStorageLayoutRequest{Name: "some/bucket"}
...
func (t *AllApiRetryWrapperTest) TestGetStorageLayout_SuccessOnFirstAttempt() {
	...
	client := newRetryWrapper(t.stallingClient, clientConfig, true)
	client.(*storageControlClientWithRetry).retryConfig.RetryDeadline = 100 * time.Microsecond
	client.(*storageControlClientWithRetry).retryConfig.TotalRetryBudget = 1000 * time.Microsecond
	req := &controlpb.GetStorageLayoutRequest{Name: "some/bucket"}
	expectedLayout := &controlpb.StorageLayout{Location: "some-location"}
