...
func readMultiple(
	ctx context.Context,
	bucket bucket.Bucket,
	reqs []*request.ReadObjectRequest) (contents [][]byte, errs []error) {
...
func (t *createTest) EmptyObject() {
	...
	listing, err := t.bucket.ListObjects(t.ctx, &request.ListObjectsRequest{})
	AssertEq(nil, err)
...
func (t *createTest) IncorrectCRC32C() {
	...
	*crc32c++
	...
	statReq := &request.StatObjectRequest{
		Name: name,
	}
...
func (t *copyTest) DestinationDoesntExist() {
	...
	statO, err := t.bucket.StatObject(
		t.ctx,
		&request.StatObjectRequest{Name: "bar"})
...
func (t *copyTest) DestinationIsSameName() {
	...
	statO, err := t.bucket.StatObject(
		t.ctx,
		&request.StatObjectRequest{Name: "foo"})
...
func (t *composeTest) ManySimpleSources() {
	...
	for _, src := range sources {
		req.Sources = append(req.Sources, request.ComposeSource{Name: src.Name})
	}
...
func (t *composeTest) DestinationExists_GenerationPreconditionNotSatisfied() {
	...
	o, err := t.bucket.StatObject(
		t.ctx,
		&request.StatObjectRequest{Name: sources[0].Name})
...
func (t *composeTest) TooManySources() {
	...
	for i := 0; i < request.MaxSourcesPerComposeRequest+1; i++ {
		req.Sources = append(req.Sources, request.ComposeSource{Name: src.Name})
	}
...
func (t *composeTest) ComponentCountLimits() {
	...
	// rounds of composing.
	AssertEq(
		request.MaxComponentCount,
		request.MaxSourcesPerComposeRequest*request.MaxSourcesPerComposeRequest)
	...
	for i := 0; i < request.MaxSourcesPerComposeRequest; i++ {
		req.Sources = append(req.Sources, request.ComposeSource{Name: small.Name})
	}

	medium, err := t.bucket.ComposeObjects(t.ctx, req)

	AssertEq(nil, err)
	AssertEq(request.MaxSourcesPerComposeRequest, medium.ComponentCount)
	AssertEq(request.MaxSourcesPerComposeRequest, medium.Size)
	...
	for i := 0; i < request.MaxSourcesPerComposeRequest; i++ {
		req.Sources = append(req.Sources, request.ComposeSource{Name: medium.Name})
	}

	large, err := t.bucket.ComposeObjects(t.ctx, req)

	AssertEq(nil, err)
	AssertEq(request.MaxComponentCount, large.ComponentCount)
	AssertEq(request.MaxComponentCount, large.Size)

	// Attempting to add one more component should fail.
	req = &request.ComposeObjectsRequest{
		DstName: "foo",
		Sources: []request.ComposeSource{
			request.ComposeSource{Name: large.Name},
			request.ComposeSource{Name: small.Name},
		},
	}
		...
		func(ctx context.Context, name string) (err error) {
			_, err = t.bucket.ComposeObjects(
				...
				&request.ComposeObjectsRequest{
					DstName: name,
					Sources: []request.ComposeSource{
						request.ComposeSource{Name: srcName},
						request.ComposeSource{Name: srcName},
					},
				})
...
func (t *readTest) Ranges_EmptyObject() {
	...
	// Test cases.
	testCases := []struct {
		br request.ByteRange
	}{
		// Empty without knowing object length
		{request.ByteRange{Start: 0, Limit: 0}},

		{request.ByteRange{Start: 1, Limit: 1}},
		{request.ByteRange{Start: 1, Limit: 0}},

		{request.ByteRange{Start: math.MaxInt64, Limit: math.MaxInt64}},
		{request.ByteRange{Start: math.MaxInt64, Limit: 17}},
		{request.ByteRange{Start: math.MaxInt64, Limit: 0}},

		{request.ByteRange{Start: math.MaxUint64, Limit: math.MaxUint64}},
		{request.ByteRange{Start: math.MaxUint64, Limit: 17}},
		{request.ByteRange{Start: math.MaxUint64, Limit: 0}},

		// Not empty without knowing object length
		{request.ByteRange{Start: 0, Limit: 1}},
		{request.ByteRange{Start: 0, Limit: 17}},
		{request.ByteRange{Start: 0, Limit: math.MaxInt64}},
		{request.ByteRange{Start: 0, Limit: math.MaxUint64}},

		{request.ByteRange{Start: 1, Limit: 2}},
		{request.ByteRange{Start: 1, Limit: 17}},
		{request.ByteRange{Start: 1, Limit: math.MaxInt64}},
		{request.ByteRange{Start: 1, Limit: math.MaxUint64}},

		{request.ByteRange{Start: math.MaxInt64, Limit: math.MaxInt64 + 1}},
		{request.ByteRange{Start: math.MaxInt64, Limit: math.MaxUint64}},
	}

	// Turn test cases into read requests.
	var reqs []*request.ReadObjectRequest
...
func (t *readTest) Ranges_NonEmptyObject() {
	...
	testCases := []struct {
		...
		// Left anchored
		{request.ByteRange{Start: 0, Limit: math.MaxUint64}, "taco"},
		{request.ByteRange{Start: 0, Limit: 5}, "taco"},
		{request.ByteRange{Start: 0, Limit: 4}, "taco"},
		{request.ByteRange{Start: 0, Limit: 3}, "tac"},
		{request.ByteRange{Start: 0, Limit: 1}, "t"},
		{request.ByteRange{Start: 0, Limit: 0}, ""},

		// Floating left edge
		{request.ByteRange{Start: 1, Limit: math.MaxUint64}, "aco"},
		{request.ByteRange{Start: 1, Limit: 5}, "aco"},
		{request.ByteRange{Start: 1, Limit: 4}, "aco"},
		{request.ByteRange{Start: 1, Limit: 2}, "a"},
		{request.ByteRange{Start: 1, Limit: 1}, ""},
		{request.ByteRange{Start: 1, Limit: 0}, ""},

		// Left edge at right edge of object
		{request.ByteRange{Start: 4, Limit: math.MaxUint64}, ""},
		{request.ByteRange{Start: 4, Limit: math.MaxInt64 + 1}, ""},
		{request.ByteRange{Start: 4, Limit: math.MaxInt64 + 0}, ""},
		{request.ByteRange{Start: 4, Limit: math.MaxInt64 - 1}, ""},
		{request.ByteRange{Start: 4, Limit: 17}, ""},
		{request.ByteRange{Start: 4, Limit: 5}, ""},
		{request.ByteRange{Start: 4, Limit: 4}, ""},
		{request.ByteRange{Start: 4, Limit: 1}, ""},
		{request.ByteRange{Start: 4, Limit: 0}, ""},

		// Left edge past right edge of object
		{request.ByteRange{Start: 5, Limit: math.MaxUint64}, ""},
		{request.ByteRange{Start: 5, Limit: 17}, ""},
		{request.ByteRange{Start: 5, Limit: 5}, ""},
		{request.ByteRange{Start: 5, Limit: 4}, ""},
		{request.ByteRange{Start: 5, Limit: 1}, ""},
		{request.ByteRange{Start: 5, Limit: 0}, ""},

		// Left edge is 2^63 - 1
		{request.ByteRange{Start: math.MaxInt64, Limit: math.MaxUint64}, ""},
		{request.ByteRange{Start: math.MaxInt64, Limit: math.MaxInt64 + 1}, ""},
		{request.ByteRange{Start: math.MaxInt64, Limit: math.MaxInt64 + 0}, ""},
		{request.ByteRange{Start: math.MaxInt64, Limit: math.MaxInt64 - 1}, ""},
		{request.ByteRange{Start: math.MaxInt64, Limit: 5}, ""},
		{request.ByteRange{Start: math.MaxInt64, Limit: 4}, ""},
		{request.ByteRange{Start: math.MaxInt64, Limit: 1}, ""},
		{request.ByteRange{Start: math.MaxInt64, Limit: 0}, ""},

		// Left edge is 2^64 - 1
		{request.ByteRange{Start: math.MaxUint64, Limit: math.MaxUint64}, ""},
		{request.ByteRange{Start: math.MaxUint64, Limit: math.MaxInt64 + 1}, ""},
		{request.ByteRange{Start: math.MaxUint64, Limit: math.MaxInt64}, ""},
		{request.ByteRange{Start: math.MaxUint64, Limit: math.MaxInt64 - 1}, ""},
		{request.ByteRange{Start: math.MaxUint64, Limit: 5}, ""},
		{request.ByteRange{Start: math.MaxInt64, Limit: 4}, ""},
		{request.ByteRange{Start: math.MaxInt64, Limit: 1}, ""},
		{request.ByteRange{Start: math.MaxInt64, Limit: 0}, ""},
	}
...
func (t *statTest) StatAfterUpdating() {
	...
	// Update the object.
	ureq := &request.UpdateObjectRequest{
		Name:        "foo",
...
func (t *updateTest) ParticularGeneration_GenerationDoesntExist() {
	...
	o, err = t.bucket.StatObject(
		t.ctx,
		&request.StatObjectRequest{Name: o.Name})
