...
type fakeMetricHandle struct {
	...
	gcsRetryCountVal    metrics.MetricAttr
}

func (m *fakeMetricHandle) GcsRetryCount(inc int64, val metrics.MetricAttr) {
...
func TestShouldRetryWithMonitoringForRetryableErrors(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, metrics.MetricAttr(tc.expectedMetricCategory), fakeMetrics.gcsRetryCountVal)
		})
	}
}
