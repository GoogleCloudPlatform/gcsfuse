...
func validateDefaultConfig(t *testing.T, mountConfig *MountConfig) {
	...
	assert.Equal(t, uint(10), mountConfig.FileCacheConfig.DownloadParallelismPerFile)
	assert.Equal(t, -1, mountConfig.FileCacheConfig.MaxDownloadParallelism)
	assert.Equal(t, uint(25), mountConfig.FileCacheConfig.ReadRequestSizeMB)
...
func (t *YamlParserTest) TestReadConfigFile_ValidConfig() {
	...
	assert.Equal(t.T(), uint(10), mountConfig.DownloadParallelismPerFile)
	assert.Equal(t.T(), -1, mountConfig.MaxDownloadParallelism)
	assert.Equal(t.T(), uint(100), mountConfig.ReadRequestSizeMB)
