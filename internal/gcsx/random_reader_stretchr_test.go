...
func (t *RandomReaderStretchrTest) Test_ReadAt_ParallelMRDReads() {
	...
	fakeMRDWrapper, err := NewMultiRangeDownloaderWrapper(t.mockBucket, t.object, &cfg.Config{}, false)
	require.NoError(t.T(), err)
	t.rr.wrapped.mrdWrapper = &fakeMRDWrapper
	t.mockBucket.On("NewMultiRangeDownloader", mock.Anything, mock.Anything).Return(fake.NewFakeMultiRangeDownloader(t.object, testContent), nil) // Parallel reads
	tasks := []struct {
		offset int64
		size   int
	}{
...
func (t *RandomReaderStretchrTest) Test_ReadAt_ValidateReadType() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func() {
			...
			t.object.Size = uint64(tc.dataSize)
			testContent := testutil.GenerateRandomBytes(int(t.object.Size))
			fakeMRDWrapper, err := NewMultiRangeDownloaderWrapperWithClock(t.mockBucket, t.object, &clock.FakeClock{}, &cfg.Config{}, false)
