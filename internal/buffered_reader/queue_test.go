...
func TestQueue_Pop(t *testing.T) {
	...
	val := q.Pop()
	assert.Equal(t, 4, val.(int))
	assert.Equal(t, 5, q.Peek())

	val = q.Pop()
	assert.Equal(t, 5, val.(int))
	assert.Nil(t, q.Peek())
...
func TestQueue_IsEmpty(t *testing.T) {
	...
	val := q.Pop()
	assert.Equal(t, 4, val.(int)) // Verify the popped value
	assert.True(t, q.IsEmpty())
...
func TestQueue_Len(t *testing.T) {
	...
	val := q.Pop()
	assert.Equal(t, 4, val.(int)) // Verify popped value
	assert.Equal(t, 2, q.Len())

	val = q.Pop()
	assert.Equal(t, 5, val.(int)) // Verify popped value
	assert.Equal(t, 1, q.Len())

	val = q.Pop()
	assert.Equal(t, 6, val.(int)) // Verify popped value
	assert.Equal(t, 0, q.Len())
