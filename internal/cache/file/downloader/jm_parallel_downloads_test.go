	...
	for _, tc := range tbl {
		t.Run(tc.name, func(t *testing.T) {
			...
			jm := NewJobManager(cache, util.DefaultFilePerm, util.DefaultDirPerm, cacheDir, 2, &cfg.FileCacheConfig{EnableParallelDownloads: true,
				...
				MaxParallelDownloads: tc.maxParallelDownloads, WriteBufferSize: 4 * 1024 * 1024})
...
func TestMultipleConcurrentDownloads(t *testing.T) {
	...
	jm := NewJobManager(cache, util.DefaultFilePerm, util.DefaultDirPerm, cacheDir, 2, &cfg.FileCacheConfig{EnableParallelDownloads: true,
		ParallelDownloadsPerFile: math.MaxInt, DownloadChunkSizeMb: 2, EnableCrc: true,
		MaxParallelDownloads: 2, WriteBufferSize: 4 * 1024 * 1024})
