	...
	for _, tc := range tbl {
		t.Run(tc.name, func(t *testing.T) {
			...
			jm := NewJobManager(cache, util.DefaultFilePerm, util.DefaultDirPerm, cacheDir, 2, &cfg.FileCacheConfig{EnableParallelDownloads: true,
				ParallelDownloadsPerFile: int64(tc.parallelDownloadsPerFile), DownloadChunkSizeMb: int64(tc.readReqSize), EnableCrc: true, MaxParallelDownloads: int64(tc.maxParallelDownloads)})
