...
func (p *DownloadTask) Execute() {
	...
	defer func() {
		if err == nil {
			...
			p.block.NotifyReady(block.BlockStatus{State: block.BlockStateDownloaded, Err: nil})
			...
			p.block.NotifyReady(block.BlockStatus{State: block.BlockStateDownloadCancelled, Err: nil})
		} else {
