...
func (t *BufferedReaderTest) TestReadAtOffsetBeyondEOF() {
	...
	resp, err := reader.ReadAt(t.ctx, buf, int64(t.object.Size+1), false)
...
func (t *BufferedReaderTest) TestReadAtEmptyBuffer() {
	...
	resp, err := reader.ReadAt(t.ctx, buf, 0, false)
...
func (t *BufferedReaderTest) TestReadAtBackwardSeekIsRandomRead() {
	...
	_, err = reader.ReadAt(t.ctx, make([]byte, 10), startOffset, false)
	require.NoError(t.T(), err)
...
func (t *BufferedReaderTest) TestReadAtForwardSeekDiscardsPreviousBlocks() {
	...
	_, err = reader.ReadAt(t.ctx, make([]byte, 1024), readOffset, false)
...
func (t *BufferedReaderTest) TestReadAtInitialDownloadFails() {
	...
	_, err = reader.ReadAt(t.ctx, buf, 0, false)
...
func (t *BufferedReaderTest) TestReadAtAwaitReadyCancelled() {
	...
	_, err = reader.ReadAt(ctx, make([]byte, 10), 0, false)
...
func (t *BufferedReaderTest) TestReadAtBlockStateDownloadFailed() {
	...
	_, err = reader.ReadAt(t.ctx, make([]byte, 10), 0, false)
...
func (t *BufferedReaderTest) TestReadAtSpanningMultipleBlocks() {
	...
	resp, err := reader.ReadAt(t.ctx, buf, readOffset, false)
...
func (t *BufferedReaderTest) TestReadAtSequentialReadAcrossBlocks() {
	...
	resp1, err := reader.ReadAt(t.ctx, buf1, 0, false)
	require.NoError(t.T(), err)
	resp2, err := reader.ReadAt(t.ctx, buf2, testPrefetchBlockSizeBytes, false)
	require.NoError(t.T(), err)
...
func (t *BufferedReaderTest) TestReadAtFallsBackAfterRandomReads() {
	...
	_, err = reader.ReadAt(t.ctx, buf, 2*testPrefetchBlockSizeBytes, false)
	...
	_, err = reader.ReadAt(t.ctx, buf, 5*testPrefetchBlockSizeBytes, false)
...
func (t *BufferedReaderTest) TestReadAtSucceedsWhenBackgroundPrefetchFailsOnGCSError() {
	...
	_, err = reader.ReadAt(t.ctx, buf, 1024, false)
...
func (t *BufferedReaderTest) TestReadAtConcurrentReads() {
	...
	for i := range numGoroutines {
		go func(index int) {
			...
			resp, err := reader.ReadAt(t.ctx, readBuf, offset, false)
...
func (t *BufferedReaderTest) TestDestroyWaitsForCallback() {
	...
	t.bucket.On("Name").Return("test-bucket").Maybe()
	resp, err := reader.ReadAt(t.ctx, buf, startOffset, false)
	require.NoError(t.T(), err)
...
func (t *BufferedReaderTest) TestConcurrentReadsOnSameBlock() {
	...
	go func() {
		defer wg.Done()
		resp1, err1 = reader.ReadAt(t.ctx, make([]byte, readSize), startOffset, false)
	}()
	go func() {
		defer wg.Done()
		resp2, err2 = reader.ReadAt(t.ctx, make([]byte, readSize), startOffset, false)
	}()
	wg.Wait()
...
func (t *BufferedReaderTest) TestEvictedBlockIsReleasedOnlyAfterCallback() {
	...
	resp1, err := reader.ReadAt(t.ctx, make([]byte, 10), 0, false)
	require.NoError(t.T(), err)
	require.NotNil(t.T(), resp1.Callback)
	...
	resp2, err := reader.ReadAt(t.ctx, make([]byte, 10), 5*testPrefetchBlockSizeBytes, false)
	require.NoError(t.T(), err)
