...
func (ps *prefetchTestSuite) SetupSuite() {
	...
	ps.workerPool, workerPoolErr = workerpool.NewStaticWorkerPool(20, 10)
	if workerPoolErr != nil {
		ps.assert.FailNowf("Failed to create worker pool: %v", workerPoolErr.Error())
	}
	ps.workerPool.Start()

	// Block pool.
	globalSemaphore := semaphore.NewWeighted(100)
	var blockPoolErr error
	ps.blockPool, blockPoolErr = block.NewPrefetchBlockPool(getDefaultBufferedReadConfig().PrefetchChunkSize, 100, globalSemaphore)
	if blockPoolErr != nil {
		ps.assert.FailNowf("Failed to create block pool: %v", blockPoolErr.Error())
	}
...
func (ps *prefetchTestSuite) TearDownSuite() {
	stime := time.Now()
	ps.workerPool.Stop()
	ps.blockPool.ClearFreeBlockChannel(true)
