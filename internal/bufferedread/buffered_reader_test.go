...
func (t *BufferedReaderTest) TestReadAtFallbackReleasesBlocks() {
	// 1. Setup
	// Configure a file large enough for multiple blocks.
	t.object.Size = uint64(10 * testPrefetchBlockSizeBytes)
	reader, err := NewBufferedReader(&BufferedReaderOptions{
		Object:             t.object,
		Bucket:             t.bucket,
		Config:             t.config,
	...
	require.NoError(t.T(), err)
	// Mock a successful download for the first block.
	t.bucket.On("NewReaderWithReadHandle", mock.Anything, mock.Anything).Return(createFakeReaderWithOffset(t.T(), int(t.object.Size), 0), nil).Once()
	t.bucket.On("Name").Return("test-bucket").Maybe()
	// Manually trigger a freshStart to populate the blockQueue.
	reader.mu.Lock()
	err = reader.freshStart(0)
	reader.mu.Unlock()
	require.NoError(t.T(), err)
	// Ensure the block is in the queue and wait for it to be ready.
	require.Equal(t.T(), 1, reader.blockQueue.Len())
	entry := reader.blockQueue.Peek()
	_, err = entry.block.AwaitReady(t.ctx)
	require.NoError(t.T(), err)
	// Force the reader into a state where it will fall back on the next random read.
	reader.randomSeekCount = reader.randomReadsThreshold

	// 2. Act
	// Perform a read at an offset that is clearly a random seek (far from the
	// currently prefetched block 0). This will increment randomSeekCount and
	// trigger the fallback.
	randomOffset := 5 * testPrefetchBlockSizeBytes
	resp, err := reader.ReadAt(t.ctx, &gcsx.ReadRequest{
		Buffer: make([]byte, 10),
		Offset: randomOffset,
	})
	...
	// 3. Assert
	assert.ErrorIs(t.T(), err, gcsx.FallbackToAnotherReader)
	assert.Nil(t.T(), resp.Data, "resp.Data should be nil on fallback")
	assert.Zero(t.T(), resp.Size, "resp.Size should be 0 on fallback")
	// The original block should have been released when the random seek was detected.
	assert.Equal(t.T(), int32(0), entry.block.RefCount(), "Block refcount should be 0 after fallback")
}
